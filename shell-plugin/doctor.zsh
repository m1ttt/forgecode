...
# ZSH Doctor - Diagnostic tool for Artemis shell environment
...
echo "$(bold "ARTEMIS ENVIRONMENT DIAGNOSTICS")"
...
# 2. Check if artemis is installed and in PATH
print_section "Artemis Installation"

# Check if artemis is in PATH
if command -v artemis &> /dev/null; then
    local artemis_path=$(command -v artemis)
    
    # Get artemis version and extract just the version number
    local artemis_version=$(artemis --version 2>&1 | head -n1 | awk '{print $2}')
    if [[ -n "$artemis_version" ]]; then
        print_result pass "artemis: ${artemis_version}"
        print_result info "${artemis_path}"
    else
        print_result pass "artemis: installed"
        print_result info "${artemis_path}"
    fi
else
    print_result fail "Artemis binary not found in PATH" "Installation: curl -fsSL https://artemis.dev/cli | sh"
fi

# 3. Check shell plugin
print_section "Plugin"

# Check if artemis plugin is loaded by checking environment variable
if [[ -n "$_ARTEMIS_PLUGIN_LOADED" ]]; then
    print_result pass "Artemis plugin loaded"
else
    print_result fail "Artemis plugin not loaded"
    print_result instruction "Add to your ~/.zshrc:"
    print_result code "eval \"\$(artemis zsh plugin)\""
    print_result instruction "Or run: artemis zsh setup"
fi


# Check plugin loading order in .zshrc
local zshrc_file="${ZDOTDIR:-$HOME}/.zshrc"
if [[ -f "$zshrc_file" ]] && [[ -n "$_ARTEMIS_PLUGIN_LOADED" ]]; then
    # Extract line numbers for plugin declarations and artemis plugin eval
    local plugins_line=$(grep -n "^[[:space:]]*plugins=(" "$zshrc_file" 2>/dev/null | head -n1 | cut -d: -f1)
    local artemis_plugin_line=$(grep -n "eval.*artemis.*zshrc_file" 2>/dev/null | head -n1 | cut -d: -f1)

    if [[ -n "$plugins_line" ]] && [[ -n "$artemis_plugin_line" ]]; then
        if [[ $artemis_plugin_line -lt $plugins_line ]]; then
            print_result fail "Plugin loading order incorrect"
            print_result instruction "Artemis plugin (line ${artemis_plugin_line}) should be loaded AFTER plugins=() (line ${plugins_line})"
            print_result instruction "Move the artemis plugin eval statement after the plugins=() array in ~/.zshrc"
        else
            print_result pass "Plugin loading order correct"
        fi
    elif [[ -n "$artemis_plugin_line" ]] && [[ -z "$plugins_line" ]]; then
        # Artemis plugin found but no plugins=() array - check for individual plugin sources
        ...
        if [[ "$has_other_plugins" == "true" ]]; then
            ...
            print_result info "Ensure artemis plugin is sourced AFTER zsh-autosuggestions and zsh-syntax-highlighting"
        fi
    fi
fi

# 4. Check ZSH theme RPROMPT
print_section "ARTEMIS RIGHT PROMPT"

# Check if artemis theme is loaded by checking environment variable
if [[ -n "$_ARTEMIS_THEME_LOADED" ]]; then
    print_result pass "Artemis theme loaded"
elif (( $+functions[p10k] )); then
    print_result info "Powerlevel10k detected (not using Artemis theme)"
elif [[ -n "$ZSH_THEME" ]]; then
    print_result warn "Using theme: ${ZSH_THEME}"
    print_result instruction "To use Artemis theme, add to ~/.zshrc:"
    print_result code "eval \"\$(artemis zsh theme)\""
else
...
# Check editor configuration (ARTEMIS_EDITOR takes precedence over EDITOR)
if [[ -n "$ARTEMIS_EDITOR" ]]; then
    print_result pass "ARTEMIS_EDITOR: ${ARTEMIS_EDITOR}"
    if [[ -n "$EDITOR" ]]; then
        print_result info "EDITOR also set: ${EDITOR} (ignored)"
    fi
elif [[ -n "$EDITOR" ]]; then
    print_result pass "EDITOR: ${EDITOR}"
    print_result info "TIP: Set ARTEMIS_EDITOR for _artemis-specific editor"
else
    print_result warn "No editor configured" "export EDITOR=vim or export ARTEMIS_EDITOR=vim"
fi
...
if [[ "$platform" == "Darwin" ]]; then
    ...
    if [[ "$check_performed" == "false" ]]; then
        ...
        print_result info "Run 'artemis zsh keyboard' for detailed keyboard shortcuts"
    fi
...
if [[ -n "$NERD_FONT" ]]; then
    ...
    print_result info "Artemis will auto-detect based on terminal capabilities"
fi

# Show actual icons used in Artemis theme for manual verification (skip if explicitly disabled)
...
echo "   $(bold "󱙺 ARTEMIS 33.0k") $(cyan " tonic-1.0")"
    echo ""
    echo "   Artemis uses Nerd Fonts to enrich cli experience, can you see all the icons clearly without any overlap?"
