...
use artemis_app::{AgentRepository, DirectoryReaderInfra, EnvironmentInfra, FileInfoInfra};
use artemis_config::ForgeConfig;
use artemis_domain::{ModelId, ProviderId, Template};
...
/// - **Global agents**: `~/.artemis/agents/*.md`
/// - **CWD agents**: `./.artemis/agents/*.md` (relative to current working
///   directory)
///
...
impl<I: FileInfoInfra + EnvironmentInfra + DirectoryReaderInfra> ForgeAgentRepository<I> {
    ...
    async fn init_default(&self) -> anyhow::Result<Vec<AgentDefinition>> {
        parse_agent_iter(
            [
                ("artemis", include_str!("agents/artemis.md")),
    ...
    AgentRepository for ForgeAgentRepository<F>
{
    ...
    async fn get_agent_infos(&self) -> anyhow::Result<Vec<artemis_domain::AgentInfo>> {
        ...
        Ok(agent_defs
            .into_iter()
            .map(|def| artemis_domain::AgentInfo {
                id: def.id,
                title: def.title,
...
mod tests {
    ...
    async fn test_parse_basic_agent() {
        let content = artemis_test_kit::fixture!("/src/fixtures/agents/basic.md").await;
    ...
    async fn test_parse_advanced_agent() {
        let content = artemis_test_kit::fixture!("/src/fixtures/agents/advanced.md").await;
