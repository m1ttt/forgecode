...
use artemis_app::dto::openai::{ListModelResponse, ProviderPipeline, Request, Response};
use artemis_app::{EnvironmentInfra, HttpInfra};
use artemis_domain::{ChatRepository, Provider};
...
impl<H: HttpInfra> OpenAIProvider<H> {
            ...
            let initiator = request.initiator.as_deref().unwrap_or_else(|| {
                ...
                let is_agent_initiated = request.messages.as_ref().is_some_and(|messages| {
                    messages.last().is_some_and(|msg| {
                        ...
                        !matches!(msg.role, artemis_app::dto::openai::Role::User)
                    })
                });
            ...
            let has_vision_content = request.messages.as_ref().is_some_and(|messages| {
                messages.iter().any(|msg| {
                    msg.content.as_ref().is_some_and(|content| match content {
                        artemis_app::dto::openai::MessageContent::Text(_) => false,
                        artemis_app::dto::openai::MessageContent::Parts(parts) => {
                            parts.iter().any(|part| {
                                matches!(part, artemis_app::dto::openai::ContentPart::ImageUrl { .. })
                            })
                        }
                    })
                })
    ...
    async fn inner_models(&self) -> Result<Vec<artemis_app::domain::Model>> {
        ...
        if self.provider.id == ProviderId::VERTEX_AI {
            ...
            match models {
                artemis_domain::ModelSource::Url(url) => {
                ...
                artemis_domain::ModelSource::Hardcoded(models) => {
    ...
    fn inner_vertex_models(&self) -> Vec<artemis_app::domain::Model> {
        static VERTEX_MODELS: LazyLock<Vec<artemis_app::domain::Model>> = LazyLock::new(|| {
...
impl<T: HttpInfra> OpenAIProvider<T> {
    ...
    pub async fn models(&self) -> Result<Vec<artemis_app::domain::Model>> {
...
mod tests {
    ...
    use anyhow::Context;
    use bytes::Bytes;
    use artemis_app::HttpInfra;
    use artemis_app::domain::{Provider, ProviderId, ProviderResponse};
    use artemis_app::dto::openai::{ContentPart, ImageUrl, Message, MessageContent, Role};
    ...
    fn openai(key: &str) -> Provider<Url> {
        Provider {
            ...
            url_params: vec![],
            models: Some(artemis_domain::ModelSource::Url(
    ...
    -> anyhow::Result<()> {
        ...
        let request = Request {
            model: Some(artemis_app::domain::ModelId::new("claude-sonnet-4-5")),
            messages: Some(vec![Message {
                role: Role::User,
                content: Some(MessageContent::Text("Hello".to_string())),
                name: None,
    ...
    -> anyhow::Result<()> {
        ...
        let request = Request {
            model: Some(artemis_app::domain::ModelId::new("gpt-4o")),
            messages: Some(vec![Message {
                role: Role::User,
                content: Some(MessageContent::Text("Hello".to_string())),
                name: None,
