...
use artemis_domain::{BoxStream, ResultStream};
            ...
            match item {
                ...
                oai::OutputItem::Reasoning(reasoning) => {
                    ...
                    if let Some(encrypted_content) = &reasoning.encrypted_content {
                        message =
                            message.add_reasoning_detail(artemis_domain::Reasoning::Full(vec![
...
fn retain_encrypted_reasoning_details(
    details: Option<Vec<artemis_domain::Reasoning>>,
) -> Option<Vec<artemis_domain::Reasoning>> {
    let details = details?;
    let encrypted: Vec<artemis_domain::Reasoning> = details
        .into_iter()
        .filter(|r| {
...
mod tests {
    ...
    use artemis_app::domain::{Content, FinishReason, Reasoning, ReasoningFull, TokenCount, Usage};
    use artemis_domain::{ChatCompletionMessage as Message, ToolCallId, ToolName};
    ...
    fn test_response_into_domain_preserves_commentary_phase() {
        ...
        assert_eq!(
            actual.phase,
            Some(artemis_app::domain::MessagePhase::Commentary)
        );
    ...
    fn test_response_into_domain_preserves_final_answer_phase() {
        ...
        assert_eq!(
            actual.phase,
            Some(artemis_app::domain::MessagePhase::FinalAnswer)
        );
    ...
    async fn test_stream_with_reasoning_text_delta() -> anyhow::Result<()> {
        ...
        assert_eq!(
            actual.reasoning_details,
            Some(vec![Reasoning::Part(vec![artemis_domain::ReasoningPart {
