...
use artemis_app::{EnvironmentInfra, HttpInfra};
use artemis_domain::{BoxStream, ChatRepository, Provider};
use artemis_infra::sanitize_headers;
    ...
    fn get_headers_for_conversation(&self, conversation_id: Option<&str>) -> Vec<(String, String)> {
        ...
        if let Some(api_key) = self
            ...
            .map(|c| match &c.auth_details {
                artemis_domain::AuthDetails::ApiKey(key) => key.as_str(),
                artemis_domain::AuthDetails::OAuthWithApiKey { api_key, .. } => api_key.as_str(),
                artemis_domain::AuthDetails::OAuth { tokens, .. } => tokens.access_token.as_str(),
                artemis_domain::AuthDetails::GoogleAdc(token) => token.as_str(),
            })
        {
        ...
        self.provider
            ...
            .for_each(|method| match method {
                artemis_domain::AuthMethod::ApiKey => {}
                artemis_domain::AuthMethod::OAuthDevice(oauth_config) => {
                ...
                artemis_domain::AuthMethod::OAuthCode(oauth_config) => {
                    if let Some(custom_headers) = &oauth_config.custom_headers {
                        custom_headers.iter().for_each(|(k, v)| {
                            headers.push((k.clone(), v.clone()));
                        });
                ...
                artemis_domain::AuthMethod::CodexDevice(oauth_config) => {
                    if let Some(custom_headers) = &oauth_config.custom_headers {
                        custom_headers.iter().for_each(|(k, v)| {
                            headers.push((k.clone(), v.clone()));
                        });
        ...
        if self.provider.id == artemis_domain::ProviderId::CODEX {
            ...
            if let Some(account_id) = self.provider.credential.as_ref().and_then(|c| {
                let key: artemis_domain::URLParam = "chatgpt_account_id".to_string().into();
...
impl<T: HttpInfra> OpenAIResponsesProvider<T> {
    pub async fn chat(
        ...
        if self.provider.id == artemis_domain::ProviderId::CODEX {
            use artemis_domain::Transformer;
                    ...
                    Ok(Event::Message(msg)) => {
                        ...
                        match result {
                            ...
                            Ok(super::response::ResponsesStreamEvent::Ping { cost }) => {
                                let usage =
                                    artemis_domain::Usage { cost: Some(cost), ..Default::default() };
                                Some(Ok(super::response::StreamItem::Message(Box::new(
                                    ChatCompletionMessage::assistant(artemis_domain::Content::part(
                                        "",
                                    ))
                                    .usage(usage),
                                ))))
    ...
    E: std::fmt::Debug + std::fmt::Display + Send + Sync + 'static,
{
    ...
    if is_retryable {
        artemis_domain::Error::Retryable(error).into()
    } else {
        error
    }
}
...
impl<F: HttpInfra + EnvironmentInfra<Config = artemis_config::ForgeConfig> + 'static> ChatRepository
    for OpenAIResponsesResponseRepository<F>
{
    ...
    async fn models(&self, provider: Provider<Url>) -> anyhow::Result<Vec<Model>> {
        match provider.models().cloned() {
            Some(artemis_domain::ModelSource::Hardcoded(models)) => Ok(models),
            Some(artemis_domain::ModelSource::Url(url)) => {
                ...
                let data: artemis_app::dto::openai::ListModelResponse =
...
mod tests {
    ...
    fn is_retryable(error: &anyhow::Error) -> bool {
        error
            .downcast_ref::<artemis_domain::Error>()
            .is_some_and(|error| matches!(error, artemis_domain::Error::Retryable(_)))
    }

    fn make_credential(provider_id: ProviderId, key: &str) -> Option<artemis_domain::AuthCredential> {
        Some(artemis_domain::AuthCredential {
            id: provider_id,
            auth_details: artemis_domain::AuthDetails::ApiKey(artemis_domain::ApiKey::from(
                key.to_string(),
            )),
    ...
    fn openai_responses(key: &str, url: &str) -> Provider<Url> {
        Provider {
            ...
            provider_type: artemis_domain::ProviderType::Llm,
            response: Some(ProviderResponse::OpenAI),
            url: Url::parse(url).unwrap(),
            credential: make_credential(ProviderId::OPENAI, key),
            custom_headers: None,
            auth_methods: vec![artemis_domain::AuthMethod::ApiKey],
            url_params: vec![],
            models: None,
        }
    }
    ...
    impl artemis_app::EnvironmentInfra for MockHttpClient {
        ...
        fn get_environment(&self) -> artemis_domain::Environment {
            use fake::{Fake, Faker};
            Faker.fake()
        }
    ...
    fn test_openai_responses_provider_new_with_oauth_with_api_key() {
        let provider = Provider {
            ...
            credential: Some(artemis_domain::AuthCredential {
                id: ProviderId::OPENAI,
                auth_details: artemis_domain::AuthDetails::OAuthWithApiKey {
                    tokens: artemis_domain::OAuthTokens::new(
                        "access-token",
                        None::<String>,
                        chrono::Utc::now() + chrono::Duration::hours(1),
                    ),
                    api_key: artemis_domain::ApiKey::from("oauth-key".to_string()),
                    config: artemis_domain::OAuthConfig {
                        auth_url: Url::parse("https://example.com/auth").unwrap(),
                        token_url: Url::parse("https://example.com/token").unwrap(),
                        client_id: artemis_domain::ClientId::from("client-id".to_string()),
                        scopes: vec![],
                        redirect_uri: None,
                        use_pkce: false,
    ...
    fn test_openai_responses_provider_new_with_oauth() {
        let provider = Provider {
            ...
            credential: Some(artemis_domain::AuthCredential {
                id: ProviderId::OPENAI,
                auth_details: artemis_domain::AuthDetails::OAuth {
                    tokens: artemis_domain::OAuthTokens::new(
                        "access-token",
                        None::<String>,
                        chrono::Utc::now() + chrono::Duration::hours(1),
    ...
    fn test_get_headers_with_oauth_device_custom_headers() {
        let provider = Provider {
            ...
            custom_headers: None,
            auth_methods: vec![artemis_domain::AuthMethod::OAuthDevice(
                artemis_domain::OAuthConfig {
                    auth_url: Url::parse("https://example.com/auth").unwrap(),
                    token_url: Url::parse("https://example.com/token").unwrap(),
                    client_id: artemis_domain::ClientId::from("client-id".to_string()),
                    scopes: vec![],
    ...
    fn test_get_headers_with_oauth_code_custom_headers() {
        let provider = Provider {
            ...
            credential: make_credential(ProviderId::OPENAI, "test-key"),
            custom_headers: None,
            auth_methods: vec![artemis_domain::AuthMethod::OAuthCode(
                artemis_domain::OAuthConfig {
                    auth_url: Url::parse("https://example.com/auth").unwrap(),
                    token_url: Url::parse("https://example.com/token").unwrap(),
                    client_id: artemis_domain::ClientId::from("client-id".to_string()),
    ...
    fn test_get_headers_with_codex_device_custom_headers() {
        let provider = Provider {
            ...
            custom_headers: None,
            auth_methods: vec![artemis_domain::AuthMethod::CodexDevice(
                artemis_domain::OAuthConfig {
                    auth_url: Url::parse(
                    ...
                    client_id: artemis_domain::ClientId::from(
                    ...
                    token_refresh_url: None,
                    custom_headers: Some(
                        [("originator".to_string(), "artemis".to_string())]
                            .into_iter()
                            .collect(),
                    ),
                    extra_auth_params: None,
    ...
    fn test_get_headers_codex_includes_chatgpt_account_id() {
        ...
        url_params.insert(
            artemis_domain::URLParam::from("chatgpt_account_id".to_string()),
            artemis_domain::URLParamValue::from("acct_test_123".to_string()),
        );
            ...
            credential: Some(artemis_domain::AuthCredential {
                ...
                auth_details: artemis_domain::AuthDetails::OAuth {
                    ...
                    config: artemis_domain::OAuthConfig {
                        ...
                        .unwrap(),
                        token_url: Url::parse("https://auth.openai.com/oauth/token").unwrap(),
                        client_id: artemis_domain::ClientId::from("app_test".to_string()),
                        scopes: vec![],
                        redirect_uri: None,
                        use_pkce: false,
                        token_refresh_url: None,
    ...
    fn test_get_headers_non_codex_does_not_include_chatgpt_account_id() {
        ...
        url_params.insert(
            artemis_domain::URLParam::from("chatgpt_account_id".to_string()),
            artemis_domain::URLParamValue::from("acct_should_not_appear".to_string()),
        );
