...
use artemis_app::domain::{Context as ChatContext, ContextMessage, MessagePhase, Role, ToolChoice};
use artemis_app::utils::enforce_strict_schema;
use artemis_domain::{Effort, ReasoningConfig, ReasoningFull};
...
mod tests {
    ...
    fn test_reasoning_config_conversion_with_effort() -> anyhow::Result<()> {
        use artemis_domain::{Effort, ReasoningConfig};
    ...
    fn test_reasoning_config_conversion_with_enabled() -> anyhow::Result<()> {
        use artemis_domain::ReasoningConfig;
    ...
    fn test_codex_request_from_context_converts_messages_tools_and_results() -> anyhow::Result<()> {
        ...
        let tool_definition =
            artemis_app::domain::ToolDefinition::new("shell").description("Run a shell command");

        let tool_call = artemis_app::domain::ToolCallFull::new("shell")
            .call_id(ToolCallId::new("call_1"))
            .arguments(artemis_app::domain::ToolCallArguments::from_json(
                r#"{"cmd":"echo hi"}"#,
            ));

        let tool_result = artemis_app::domain::ToolResult::new("shell")
    ...
    fn fixture_tool_definition(name: &str) -> artemis_app::domain::ToolDefinition {
        artemis_app::domain::ToolDefinition::new(name).description("Test tool")
    }

    fn fixture_tool_call(name: &str, call_id: &str, args: &str) -> artemis_app::domain::ToolCallFull {
        artemis_app::domain::ToolCallFull::new(name)
            .call_id(ToolCallId::new(call_id))
            .arguments(artemis_app::domain::ToolCallArguments::from_json(args))
    }
    ...
    fn test_codex_request_tools_snapshot() -> anyhow::Result<()> {
        ...
        let tool = artemis_app::domain::ToolDefinition::new("shell")
    ...
    fn test_codex_request_all_catalog_tools_snapshot() -> anyhow::Result<()> {
        use artemis_app::domain::ToolCatalog;
    ...
    fn test_codex_request_sets_prompt_cache_key_from_conversation_id() -> anyhow::Result<()> {
        use artemis_domain::ConversationId;
    ...
    -> anyhow::Result<()> {
        use artemis_domain::ReasoningFull;
    ...
    fn test_codex_request_with_temperature() -> anyhow::Result<()> {
        use artemis_app::domain::Temperature;
    ...
    fn test_codex_request_with_empty_assistant_message() -> anyhow::Result<()> {
        ...
        let context = ChatContext::default()
            ...
            .add_message(ContextMessage::tool_result(
                artemis_app::domain::ToolResult::new("shell")
                    .call_id(Some(ToolCallId::new("call_1")))
                    .success("output"),
            ));
    ...
    fn test_codex_request_with_tool_call_missing_call_id_returns_error() {
        let tool_call = artemis_app::domain::ToolCallFull::new("shell").arguments(
            artemis_app::domain::ToolCallArguments::from_json(r#"{"cmd":"ls"}"#),
        );
    ...
    fn test_codex_request_with_tool_result_missing_call_id_returns_error() {
        let context = ChatContext::default()
            .add_message(ContextMessage::user("Run command", None))
            .add_message(ContextMessage::tool_result(
                artemis_app::domain::ToolResult::new("shell").success("output"),
            ));
    ...
    fn test_codex_request_preserves_phase_on_assistant_message() -> anyhow::Result<()> {
        use artemis_app::domain::{MessagePhase, TextMessage};
        use artemis_domain::Role;
        ...
        let context = ChatContext::default()
            .add_message(ContextMessage::user("Hello", None))
            .add_entry(artemis_app::domain::MessageEntry::from(ContextMessage::Text(
                assistant_msg,
            )))
            .add_message(ContextMessage::user("Continue", None));
