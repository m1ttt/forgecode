...
use artemis_infra::CacacheStorage;
...
impl<F: EnvironmentInfra<Config = artemis_config::ForgeConfig> + HttpInfra> ForgeChatRepository<F> {
...
impl<F: EnvironmentInfra<Config = artemis_config::ForgeConfig> + HttpInfra + Sync> ChatRepository
...
impl<F: HttpInfra + EnvironmentInfra<Config = artemis_config::ForgeConfig> + Sync> ProviderRouter<F> {
    async fn chat(
        &self,
        model_id: &ModelId,
        context: Context,
