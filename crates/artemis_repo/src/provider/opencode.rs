...
use artemis_domain::ChatRepository;
use url::Url;
...
impl<F: HttpInfra + EnvironmentInfra<Config = artemis_config::ForgeConfig> + Sync>
    OpenCodeZenResponseRepository<F>
{
    ...
    pub async fn models(&self, provider: Provider<Url>) -> Result<Vec<Model>> {
        ...
        if let Some(models) = provider.models() {
            match models {
                artemis_domain::ModelSource::Hardcoded(models) => Ok(models.clone()),
                artemis_domain::ModelSource::Url(_) => {
