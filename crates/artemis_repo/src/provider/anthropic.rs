...
use artemis_domain::{ChatRepository, Provider, ProviderId};
    ...
    fn get_headers(&self) -> Vec<(String, String)> {
        ...
        let api_key = self
            ...
            .map(|c| match &c.auth_details {
                ...
                artemis_domain::AuthDetails::GoogleAdc(api_key) => api_key.as_str(),
            });
...
impl<F: HttpInfra> AnthropicResponseRepository<F> {
    ...
    fn create_client(&self, provider: Provider<Url>) -> anyhow::Result<Anthropic<F>> {
        ...
        let is_oauth = provider
            .credential
            .as_ref()
            .map(|c| matches!(c.auth_details, artemis_domain::AuthDetails::OAuth { .. }))
            .unwrap_or(false);
...
mod tests {
    ...
    fn create_anthropic(base_url: &str) -> anyhow::Result<Anthropic<MockHttpClient>> {
        ...
        let provider = Provider {
            id: artemis_app::domain::ProviderId::ANTHROPIC,
            provider_type: artemis_domain::ProviderType::Llm,
            response: Some(artemis_app::domain::ProviderResponse::Anthropic),
            url: chat_url,
            ...
            url_params: vec![],
            models: Some(artemis_domain::ModelSource::Url(model_url)),
            custom_headers: None,
        };
    ...
    async fn test_url_for_models() {
        ...
        let provider = Provider {
            ...
            auth_methods: vec![artemis_domain::AuthMethod::ApiKey],
            url_params: vec![],
            models: Some(artemis_domain::ModelSource::Url(model_url.clone())),
            custom_headers: None,
        };
    ...
    fn test_vertex_ai_removes_output_format() {
        use artemis_domain::ResponseFormat;
        ...
        let provider = Provider {
            ...
            credential: Some(artemis_domain::AuthCredential {
                id: ProviderId::VERTEX_AI_ANTHROPIC,
                auth_details: artemis_domain::AuthDetails::GoogleAdc(artemis_domain::ApiKey::from(
                    "test-token".to_string(),
                )),
                url_params: std::collections::HashMap::new(),
            }),
            auth_methods: vec![artemis_domain::AuthMethod::GoogleAdc],
            url_params: vec![],
            models: Some(artemis_domain::ModelSource::Hardcoded(vec![])),
            custom_headers: None,
        };
