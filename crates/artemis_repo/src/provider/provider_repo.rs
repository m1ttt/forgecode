...
use bytes::Bytes;
use artemis_app::domain::{ProviderId, ProviderResponse};
use artemis_app::{EnvironmentInfra, FileReaderInfra, FileWriterInfra, HttpInfra};
use artemis_domain::{
...
enum Models {
    ...
    Hardcoded(Vec<artemis_app::domain::Model>),
}
...
struct ProviderConfig {
    ...
    #[merge(strategy = merge::vec::append)]
    auth_methods: Vec<artemis_domain::AuthMethod>,
    #[serde(default)]
    #[merge(strategy = overwrite)]
...
impl From<artemis_config::ProviderUrlParam> for UrlParamVarConfig {
    fn from(param: artemis_config::ProviderUrlParam) -> Self {
...
impl From<artemis_config::ProviderEntry> for ProviderConfig {
    fn from(entry: artemis_config::ProviderEntry) -> Self {
        let provider_type = match entry.provider_type {
            Some(artemis_config::ProviderTypeEntry::ContextEngine) => {
                artemis_domain::ProviderType::ContextEngine
            }
            Some(artemis_config::ProviderTypeEntry::Llm) | None => artemis_domain::ProviderType::Llm,
        };

        let auth_methods = if entry.auth_methods.is_empty() {
            vec![artemis_domain::AuthMethod::ApiKey]
        } else {
            entry
                .auth_methods
                .into_iter()
                .map(|m| match m {
                    artemis_config::ProviderAuthMethod::ApiKey => artemis_domain::AuthMethod::ApiKey,
                    artemis_config::ProviderAuthMethod::GoogleAdc => {
                        artemis_domain::AuthMethod::GoogleAdc
                    }
                })
                .collect()
        };
        ...
        let response_type = entry.response_type.map(|r| match r {
            artemis_config::ProviderResponseType::OpenAI => ProviderResponse::OpenAI,
            artemis_config::ProviderResponseType::OpenAIResponses => {
                ProviderResponse::OpenAIResponses
            }
            artemis_config::ProviderResponseType::Anthropic => ProviderResponse::Anthropic,
            artemis_config::ProviderResponseType::Bedrock => ProviderResponse::Bedrock,
            artemis_config::ProviderResponseType::Google => ProviderResponse::Google,
            artemis_config::ProviderResponseType::OpenCode => ProviderResponse::OpenCode,
        });
...
impl From<&ProviderConfig> for artemis_domain::ProviderTemplate {
    fn from(config: &ProviderConfig) -> Self {
        let models = config.models.as_ref().map(|m| match m {
            Models::Url(model_url_template) => artemis_domain::ModelSource::Url(
                artemis_domain::Template::<artemis_domain::URLParameters>::new(model_url_template),
            ),
            Models::Hardcoded(model_list) => {
                artemis_domain::ModelSource::Hardcoded(model_list.clone())
            }
        });
        ...
        Provider {
            ...
            url: artemis_domain::Template::<artemis_domain::URLParameters>::new(&config.url),
...
impl<F: EnvironmentInfra<Config = artemis_config::ForgeConfig> + HttpInfra>
...
impl<
    F: EnvironmentInfra<Config = artemis_config::ForgeConfig>
        + FileReaderInfra
        + FileWriterInfra
        + HttpInfra,
> ForgeProviderRepository<F>
{
    ...
    async fn create_provider(
        &self,
        config: &ProviderConfig,
    ) -> anyhow::Result<artemis_domain::ProviderTemplate> {
        ...
        if (credential.id == artemis_domain::ProviderId::VERTEX_AI
            || credential.id == artemis_domain::ProviderId::VERTEX_AI_ANTHROPIC)
            && let artemis_domain::AuthDetails::ApiKey(ref api_key) = credential.auth_details
    ...
    async fn refresh_google_adc_credential(
        &self,
        original_credential: &artemis_domain::AuthCredential,
    ) -> anyhow::Result<artemis_domain::AuthCredential> {
        ...
        Ok(artemis_domain::AuthCredential::new_api_key(
            original_credential.id.clone(),
            artemis_domain::ApiKey::from(access_token.token),
        )
...
> ProviderRepository for ForgeProviderRepository<F>
{
    ...
    async fn get_provider(&self, id: ProviderId) -> anyhow::Result<artemis_domain::ProviderTemplate> {
...
#[cfg(test)]
mod tests {
    use artemis_app::domain::{AuthMethod, ProviderResponse};
...
mod env_tests {
    ...
    use artemis_app::domain::{
        ChatCompletionMessage, Context, Environment, Model, ModelId, ResultStream,
    };
    use artemis_domain::{AnyProvider, ChatRepository, ProviderTemplate};
    use pretty_assertions::assert_eq;
    use url::Url;
    ...
    impl EnvironmentInfra for MockInfra {
        ...
        async fn update_environment(
            &self,
            _ops: Vec<artemis_domain::ConfigOperation>,
        ) -> anyhow::Result<()> {
            Ok(())
        }
    ...
    impl FileReaderInfra for MockInfra {
        ...
        async fn range_read_utf8(
            ...
            _start_line: u64,
            _end_line: u64,
        ) -> anyhow::Result<(String, artemis_domain::FileInfo)> {
            Err(anyhow::anyhow!("File not found"))
        }
    }
    ...
    impl ProviderRepository for MockInfra {
        ...
        async fn upsert_credential(
            &self,
            _credential: artemis_domain::AuthCredential,
        ) -> anyhow::Result<()> {
            Ok(())
        }
        ...
        async fn get_credential(
            &self,
            _id: &ProviderId,
        ) -> anyhow::Result<Option<artemis_domain::AuthCredential>> {
            Ok(None)
        }
        ...
        async fn migrate_env_credentials(
            &self,
        ) -> anyhow::Result<Option<artemis_domain::MigrationResult>> {
            Ok(None)
        }
    }
    ...
    async fn test_create_azure_provider_with_handlebars_urls() {
        ...
        assert_eq!(
            provider
                ...
                .and_then(|c| match &c.auth_details {
                    artemis_domain::AuthDetails::ApiKey(key) => Some(key.to_string()),
                    _ => None,
                }),
        ...
        match &provider.models.as_ref().unwrap() {
            artemis_domain::ModelSource::Url(model_template) => {
                assert_eq!(
                    model_template.template,
                    "https://{{AZURE_RESOURCE_NAME}}.openai.azure.com/openai/models?api-version={{AZURE_API_VERSION}}"
                );
            }
            artemis_domain::ModelSource::Hardcoded(_) => panic!("Expected ModelSource::Url variant"),
        }
    }
