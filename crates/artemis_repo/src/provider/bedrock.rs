...
use artemis_config::RetryConfig;
use artemis_domain::{
...
impl BedrockProvider {
    ...
    pub fn new(provider: Provider<Url>) -> Result<Self> {
        ...
        let region_param: artemis_domain::URLParam = "AWS_REGION".to_string().into();
    ...
    pub async fn chat(
        ...
        let output = self
            ...
            .map_err(|sdk_error| {
                ...
                if is_retryable {
                    artemis_domain::Error::Retryable(anyhow::anyhow!("{}", source)).into()
                } else {
            ...
            match event_stream.recv().await {
                ...
                Err(stream_error) => {
                    ...
                    let error = if is_retryable {
                        artemis_domain::Error::Retryable(anyhow::anyhow!(
    ...
    pub async fn models(&self) -> Result<Vec<Model>> {
        ...
        match &self.provider.models {
            Some(artemis_domain::ModelSource::Hardcoded(models)) => Ok(models.clone()),
            _ => Ok(vec![]),
        }
    }
}
...
impl IntoDomain for aws_sdk_bedrockruntime::types::ConverseStreamOutput {
    type Domain = artemis_domain::ChatCompletionMessage;
                            ...
                            match reasoning {
                                aws_sdk_bedrockruntime::types::ReasoningContentBlockDelta::Text(
                                    ...
                                    ChatCompletionMessage::default()
                                        ...
                                        .add_reasoning_detail(artemis_domain::Reasoning::Part(vec![
                                            artemis_domain::ReasoningPart {
                                                text: Some(text),
                                                signature: None,
                                                ..Default::default()
                                            },
                                ...
                                aws_sdk_bedrockruntime::types::ReasoningContentBlockDelta::Signature(
                                    ...
                                    ChatCompletionMessage::default().add_reasoning_detail(
                                        artemis_domain::Reasoning::Part(vec![
                                            artemis_domain::ReasoningPart {
                                                text: None,
                                                signature: Some(sig),
                                                ..Default::default()
            ...
            ConverseStreamOutput::Metadata(metadata) => {
                ...
                let usage = metadata.usage.map(|u| {
                    ...
                    artemis_domain::Usage {
                        prompt_tokens: artemis_domain::TokenCount::Actual(u.input_tokens as usize),
                        completion_tokens: artemis_domain::TokenCount::Actual(
                            u.output_tokens as usize,
                        ),
                        total_tokens: artemis_domain::TokenCount::Actual(u.total_tokens as usize),
                        cached_tokens: artemis_domain::TokenCount::Actual(cached_tokens as usize),
                        ..Default::default()
                    }
                });
...
impl FromDomain<artemis_domain::Context>
    for aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput
{
    fn from_domain(context: artemis_domain::Context) -> anyhow::Result<Self> {
        ...
        let system: Vec<SystemContentBlock> = context
            ...
            .filter_map(|msg| match &msg.message {
                artemis_domain::ContextMessage::Text(text_msg)
                    if text_msg.has_role(artemis_domain::Role::System) =>
                {
        ...
        let messages: Vec<Message> = {
            ...
            let mut pending_tool_results: Vec<artemis_domain::ContextMessage> = Vec::new();

            for message in context.messages.into_iter() {
                if message.has_role(artemis_domain::Role::System) {
                    continue;
                }

                match &message.message {
                    artemis_domain::ContextMessage::Tool(_) => {
        ...
        let tool_config = if !context.tools.is_empty() {
            ...
            let choice = context
                .tool_choice
                .filter(|c| !matches!(c, artemis_domain::ToolChoice::None))
        ...
        let adjusted_top_p = if has_thinking {
            ...
            context.top_p.map(|p| {
                let value = p.value();
                if value < 0.95 {
                    artemis_domain::TopP::new(0.95).unwrap()
                } else {
                    p
                }
            })
...
impl FromDomain<Vec<artemis_domain::ContextMessage>> for aws_sdk_bedrockruntime::types::Message {
    fn from_domain(tool_results: Vec<artemis_domain::ContextMessage>) -> anyhow::Result<Self> {
        ...
        for msg in tool_results {
            match msg {
                artemis_domain::ContextMessage::Tool(tool_result) => {
...
impl FromDomain<artemis_domain::ContextMessage> for aws_sdk_bedrockruntime::types::Message {
    fn from_domain(msg: artemis_domain::ContextMessage) -> anyhow::Result<Self> {
        ...
        match msg {
            artemis_domain::ContextMessage::Text(text_msg) => {
                ...
                if text_msg.role == artemis_domain::Role::Assistant
                ...
                let role = match text_msg.role {
                    artemis_domain::Role::User => ConversationRole::User,
                    artemis_domain::Role::Assistant => ConversationRole::Assistant,
                    artemis_domain::Role::System => {
            ...
            artemis_domain::ContextMessage::Image(img) => {
...
impl FromDomain<artemis_domain::ToolCallArguments> for aws_smithy_types::Document {
    fn from_domain(args: artemis_domain::ToolCallArguments) -> anyhow::Result<Self> {
        use anyhow::Context as _;
...
impl FromDomain<artemis_domain::ToolDefinition> for aws_sdk_bedrockruntime::types::Tool {
    fn from_domain(tool: artemis_domain::ToolDefinition) -> anyhow::Result<Self> {
...
impl FromDomain<artemis_domain::ToolChoice> for aws_sdk_bedrockruntime::types::ToolChoice {
    fn from_domain(choice: artemis_domain::ToolChoice) -> anyhow::Result<Self> {
        use aws_sdk_bedrockruntime::types::{
            AnyToolChoice, AutoToolChoice, SpecificToolChoice, ToolChoice,
        };

        let bedrock_choice = match choice {
            artemis_domain::ToolChoice::Auto => ToolChoice::Auto(AutoToolChoice::builder().build()),
            artemis_domain::ToolChoice::Required => ToolChoice::Any(AnyToolChoice::builder().build()),
            artemis_domain::ToolChoice::Call(tool_name) => ToolChoice::Tool(
                SpecificToolChoice::builder()
                    .name(tool_name.to_string())
                    .build()
                    .map_err(|e| anyhow::anyhow!("Failed to build tool choice: {}", e))?,
            ),
            artemis_domain::ToolChoice::None => {
...
#[cfg(test)]
mod tests {
    use artemis_domain::InputModality;
    ...
    async fn test_models_returns_hardcoded() {
        use artemis_domain::{Model, ModelId, ModelSource};
    ...
    fn test_into_domain_content_block_delta_text() {
        ...
        use artemis_domain::{ChatCompletionMessage, Content};
    ...
    fn test_into_domain_content_block_start_tool_use() {
        ...
        use artemis_domain::{ChatCompletionMessage, Content, ToolCallId, ToolCallPart, ToolName};
    ...
    fn test_into_domain_message_stop_end_turn() {
        ...
        use artemis_domain::{ChatCompletionMessage, Content, FinishReason};
    ...
    fn test_into_domain_metadata_with_usage() {
        use aws_sdk_bedrockruntime::types::ConverseStreamOutput;
        use artemis_domain::{ChatCompletionMessage, Content, TokenCount};
        ...
        let actual = fixture.into_domain();
        let expected =
            ChatCompletionMessage::assistant(Content::part("")).usage(artemis_domain::Usage {
    ...
    fn test_from_domain_tool_choice_auto() {
        ...
        let fixture = artemis_domain::ToolChoice::Auto;
    ...
    fn test_from_domain_tool_choice_required() {
        ...
        let fixture = artemis_domain::ToolChoice::Required;
        let actual = ToolChoice::from_domain(fixture).unwrap();
    ...
    fn test_from_domain_tool_choice_call() {
        ...
        let fixture = artemis_domain::ToolChoice::Call(artemis_domain::ToolName::new("my_tool"));
        let actual = ToolChoice::from_domain(fixture).unwrap();
    ...
    fn test_from_domain_tool_definition() {
        ...
        use artemis_domain::ToolDefinition;
        ...
        let fixture = ToolDefinition {
            name: artemis_domain::ToolName::new("test_tool"),
    ...
    fn test_from_domain_context_message_text_user() {
        ...
        use artemis_domain::{ContextMessage, Role, TextMessage};
    ...
    fn test_from_domain_context_message_text_assistant() {
        use aws_sdk_bedrockruntime::types::{ContentBlock, ConversationRole, Message};
        use artemis_domain::{ContextMessage, TextMessage};
    ...
    fn test_from_domain_context_message_text_assistant_reasoning_uses_message_signature() {
        ...
        use artemis_domain::{ContextMessage, ReasoningFull, TextMessage};
        use pretty_assertions::assert_eq;
    ...
    fn test_from_domain_context_message_tool_result() {
        ...
        use artemis_domain::{ContextMessage, ToolCallId, ToolResult};
    ...
    fn test_from_domain_context_with_system_messages() {
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
        use artemis_domain::{Context, ContextMessage, Role, TextMessage};
    ...
    fn test_from_domain_context_with_temperature() {
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
        use artemis_domain::{Context, Temperature};
    ...
    fn test_from_domain_context_with_reasoning_adjusts_top_p() {
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
        use artemis_domain::{Context, ReasoningConfig, TopP};
    ...
    fn test_from_domain_context_with_reasoning_enabled() {
        use aws_sdk_bedrockruntime::operation::converse_stream::ConverseStreamInput;
        use artemis_domain::{Context, ReasoningConfig};
