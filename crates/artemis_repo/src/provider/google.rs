...
use artemis_app::domain::{ChatCompletionMessage, Context, Model, ModelId, ResultStream};
use artemis_app::dto::google::{EventData, Request};
...
struct Google<T> {
    ...
    api_key: String,
    chat_url: Url,
    models: artemis_domain::ModelSource<Url>,
            ...
            artemis_domain::ModelSource::Url(url) => {
                ...
                if status.is_success() {
                    ...
                    struct ModelsResponse {
                        models: Vec<artemis_app::dto::google::Model>,
                    }
...
impl<F: HttpInfra> GoogleResponseRepository<F> {
    ...
    fn create_client(&self, provider: &Provider<Url>) -> anyhow::Result<Google<F>> {
        ...
        let (token, use_api_key_header) = match creds {
            artemis_domain::AuthDetails::ApiKey(api_key) => (api_key.as_str().to_string(), true),
            artemis_domain::AuthDetails::GoogleAdc(token) => (token.as_str().to_string(), false),
            artemis_domain::AuthDetails::OAuth { tokens, .. } => {
                (tokens.access_token.as_str().to_string(), false)
            }
            artemis_domain::AuthDetails::OAuthWithApiKey { api_key, .. } => {
...
mod tests {
    ...
    fn create_google(base_url: &str) -> anyhow::Result<Google<MockHttpClient>> {
        ...
        Ok(Google::new(
            ...
            chat_url,
            artemis_domain::ModelSource::Url(model_url),
            true,
        ))
    }
    ...
    async fn test_url_for_models() {
        ...
        let google = Google::new(
            ...
            chat_url,
            artemis_domain::ModelSource::Url(model_url.clone()),
            true,
        );
        match &google.models {
            artemis_domain::ModelSource::Url(url) => {
    ...
    async fn test_request_conversion() {
        ...
        // `use artemis_app::dto::google::{EventData, Request};` is already there.
