...
impl TryFrom<FileRefNode> for artemis_domain::FileHash {
    type Error = anyhow::Error;

    fn try_from(file_ref_node: FileRefNode) -> Result<Self> {
        let data = file_ref_node.data.context("Missing data in FileRefNode")?;
        Ok(artemis_domain::FileHash { path: data.path, hash: data.file_hash })
    }
}
...
impl<I: GrpcInfra> WorkspaceIndexRepository for ForgeContextEngineRepository<I> {
    ...
    async fn search(
        &self,
        search_query: &artemis_domain::CodeSearchQuery<'_>,
        auth_token: &artemis_domain::ApiKey,
    ) -> Result<Vec<Node>> {
            ...
            .filter_map(|query_item| {
                ...
                let code_node = match node_data.kind? {
                    node_data::Kind::FileChunk(chunk) => {
                        artemis_domain::NodeData::FileChunk(artemis_domain::FileChunk {
                    ...
                    node_data::Kind::File(file) => {
                        artemis_domain::NodeData::File(artemis_domain::FileNode {
                            file_path: file.path,
                    ...
                    node_data::Kind::FileRef(file_ref) => {
                        artemis_domain::NodeData::FileRef(artemis_domain::FileRef {
                            file_path: file_ref.path,
                            file_hash: file_ref.file_hash,
                        })
                    }
                    node_data::Kind::Note(note) => {
                        artemis_domain::NodeData::Note(artemis_domain::Note { content: note.content })
                    }
                    node_data::Kind::Task(task) => {
                        artemis_domain::NodeData::Task(artemis_domain::Task { task: task.task })
                    }
                };
    ...
    async fn list_workspace_files(
        ...
        auth_token: &artemis_domain::ApiKey,
    ) -> Result<Vec<artemis_domain::FileHash>> {
