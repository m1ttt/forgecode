...
pub use artemis_infra::CacacheStorage;
...
#[async_trait::async_trait]
impl<F: EnvironmentInfra<Config = artemis_config::ForgeConfig> + Send + Sync> EnvironmentInfra
    for ForgeRepo<F>
{
    ...
    fn update_environment(
        &self,
        ops: Vec<artemis_domain::ConfigOperation>,
    ...
    F: CommandInfra + Send + Sync,
{
    ...
    async fn start_command(
        ...
        working_dir: PathBuf,
        env_vars: Option<Vec<String>>,
    ) -> anyhow::Result<artemis_domain::CommandStart> {
        self.infra
            .start_command(command, working_dir, env_vars)
            .await
    }

    async fn poll_command(&self, job_id: u64) -> anyhow::Result<artemis_domain::CommandJobSnapshot> {
        self.infra.poll_command(job_id).await
    }
    ...
    ) -> anyhow::Result<artemis_domain::CommandJobSnapshot> {
        self.infra.wait_command(job_id, timeout_ms).await
    }

    async fn kill_command(&self, job_id: u64) -> anyhow::Result<artemis_domain::CommandKill> {
        self.infra.kill_command(job_id).await
    }
}
...
impl<F: StrategyFactory> StrategyFactory for ForgeRepo<F> {
    ...
    fn create_auth_strategy(
        ...
        auth_method: artemis_domain::AuthMethod,
        required_params: Vec<artemis_domain::URLParamSpec>,
...
#[async_trait::async_trait]
impl<F: GrpcInfra + Send + Sync> artemis_domain::WorkspaceIndexRepository for ForgeRepo<F> {
    async fn authenticate(&self) -> anyhow::Result<artemis_domain::WorkspaceAuth> {
    ...
    async fn create_workspace(
        ...
        auth_token: &artemis_domain::ApiKey,
    ) -> anyhow::Result<artemis_domain::WorkspaceId> {
        self.codebase_repo
    ...
    async fn upload_files(
        &self,
        upload: &artemis_domain::FileUpload,
        auth_token: &artemis_domain::ApiKey,
    ) -> anyhow::Result<artemis_domain::FileUploadInfo> {
        self.codebase_repo.upload_files(upload, auth_token).await
    }

    async fn search(
        &self,
        query: &artemis_domain::CodeSearchQuery<'_>,
        auth_token: &artemis_domain::ApiKey,
    ) -> anyhow::Result<Vec<artemis_domain::Node>> {
    ...
    async fn list_workspaces(
        &self,
        auth_token: &artemis_domain::ApiKey,
    ) -> anyhow::Result<Vec<artemis_domain::WorkspaceInfo>> {
        self.codebase_repo.list_workspaces(auth_token).await
    }

    async fn get_workspace(
        &self,
        workspace_id: &artemis_domain::WorkspaceId,
        auth_token: &artemis_domain::ApiKey,
    ) -> anyhow::Result<Option<artemis_domain::WorkspaceInfo>> {
        self.codebase_repo
    ...
    async fn list_workspace_files(
        &self,
        workspace: &artemis_domain::WorkspaceFiles,
        auth_token: &artemis_domain::ApiKey,
    ) -> anyhow::Result<Vec<artemis_domain::FileHash>> {
        self.codebase_repo
    ...
    async fn delete_files(
        &self,
        deletion: &artemis_domain::FileDeletion,
        auth_token: &artemis_domain::ApiKey,
    ) -> anyhow::Result<()> {
...
#[async_trait::async_trait]
impl<F: GrpcInfra + Send + Sync> artemis_domain::ValidationRepository for ForgeRepo<F> {
    async fn validate_file(
        &self,
        path: impl AsRef<std::path::Path> + Send,
        content: &str,
    ) -> anyhow::Result<Vec<artemis_domain::SyntaxError>> {
...
impl<F: artemis_domain::ConsoleWriter> artemis_domain::ConsoleWriter for ForgeRepo<F> {
