...
use diesel::prelude::*;
use artemis_domain::{Conversation, ConversationId, ConversationRepository, WorkspaceHash};
...
mod tests {
    ...
    fn test_context_record_conversion_preserves_all_fields() {
        ...
        let reasoning = artemis_domain::ReasoningConfig {
        ...
        let messages = vec![
            ...
            .into(),
            artemis_domain::MessageEntry {
                message: ContextMessage::Text(artemis_domain::TextMessage {
                    role: Role::Assistant,
                    ...
                    tool_calls: Some(vec![ToolCallFull {
                        ...
                        arguments: artemis_domain::ToolCallArguments::from(
                            serde_json::json!({"param": "value"}),
                        ),
                        thought_signature: None,
                    }]),
                    model: Some(artemis_domain::ModelId::from("gpt-4")),
                    thought_signature: None,
                ...
                usage: Some(Usage {
                    prompt_tokens: artemis_domain::TokenCount::Actual(100),
                    completion_tokens: artemis_domain::TokenCount::Actual(50),
                    total_tokens: artemis_domain::TokenCount::Actual(150),
                    cached_tokens: artemis_domain::TokenCount::Actual(0),
                    cost: Some(0.001),
                }),
            },
        ];
        ...
        let fixture = Context::default()
            ...
            .temperature(artemis_domain::Temperature::new(0.7).unwrap())
            .top_p(artemis_domain::TopP::new(0.9).unwrap())
            .top_k(artemis_domain::TopK::new(50).unwrap())
    ...
    fn test_legacy_tool_value_pair_deserialization() {
        ...
        let actual: artemis_domain::ToolOutput = record.try_into().unwrap();
        ...
        assert_eq!(
            actual.values[0],
            artemis_domain::ToolValue::Text("XML content for LLM".to_string())
        );
    }
    ...
    fn test_legacy_tool_value_markdown_deserialization() {
        ...
        assert_eq!(
            actual.values[0],
            artemis_domain::ToolValue::Text("# Heading - Some bold text".to_string())
        );
    }
    ...
    fn test_legacy_tool_value_file_diff_deserialization() {
        ...
        assert_eq!(
            actual.values[0],
            artemis_domain::ToolValue::Text("[File diff: /src/main.rs]".to_string())
        );
    }
}
