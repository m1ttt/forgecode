...
use artemis_domain::{Context, ConversationId};
...
impl From<&artemis_domain::ModelId> for ModelIdRecord {
    fn from(id: &artemis_domain::ModelId) -> Self {
        Self(id.to_string())
    }
}

impl From<ModelIdRecord> for artemis_domain::ModelId {
    fn from(record: ModelIdRecord) -> Self {
        artemis_domain::ModelId::from(record.0)
    }
}
...
impl From<&artemis_domain::Image> for ImageRecord {
    fn from(image: &artemis_domain::Image) -> Self {
        Self {
...
impl From<ImageRecord> for artemis_domain::Image {
    fn from(record: ImageRecord) -> Self {
        artemis_domain::Image::new_base64(
            record
                .url
...
impl From<&artemis_domain::ToolCallId> for ToolCallIdRecord {
    fn from(id: &artemis_domain::ToolCallId) -> Self {
        Self(id.as_str().to_string())
    }
}

impl From<ToolCallIdRecord> for artemis_domain::ToolCallId {
    fn from(record: ToolCallIdRecord) -> Self {
        artemis_domain::ToolCallId::new(record.0)
    }
}
...
impl From<&artemis_domain::ToolCallArguments> for ToolCallArgumentsRecord {
    fn from(args: &artemis_domain::ToolCallArguments) -> Self {
        // Serialize to JSON to capture both Parsed and Unparsed variants
        Self(serde_json::to_value(args).unwrap_or_default())
    }
}

impl From<ToolCallArgumentsRecord> for artemis_domain::ToolCallArguments {
...
impl From<&artemis_domain::ToolName> for ToolNameRecord {
    fn from(name: &artemis_domain::ToolName) -> Self {
        Self(name.to_string())
    }
}

impl From<ToolNameRecord> for artemis_domain::ToolName {
    fn from(record: ToolNameRecord) -> Self {
        artemis_domain::ToolName::new(record.0)
    }
}
...
impl From<&artemis_domain::ToolCallFull> for ToolCallFullRecord {
    fn from(call: &artemis_domain::ToolCallFull) -> Self {
        Self {
...
impl From<ToolCallFullRecord> for artemis_domain::ToolCallFull {
    fn from(record: ToolCallFullRecord) -> Self {
        artemis_domain::ToolCallFull {
...
impl From<&artemis_domain::ReasoningFull> for ReasoningFullRecord {
    fn from(reasoning: &artemis_domain::ReasoningFull) -> Self {
        Self {
...
impl From<ReasoningFullRecord> for artemis_domain::ReasoningFull {
    fn from(record: ReasoningFullRecord) -> Self {
        artemis_domain::ReasoningFull {
            text: record.text,
...
impl From<&artemis_domain::TokenCount> for TokenCountRecord {
    fn from(count: &artemis_domain::TokenCount) -> Self {
        match count {
            artemis_domain::TokenCount::Actual(n) => Self::Actual(*n),
            artemis_domain::TokenCount::Approx(n) => Self::Approx(*n),
        }
    }
}

impl From<TokenCountRecord> for artemis_domain::TokenCount {
...
impl From<&artemis_domain::Usage> for UsageRecord {
    fn from(usage: &artemis_domain::Usage) -> Self {
        Self {
...
impl From<UsageRecord> for artemis_domain::Usage {
    fn from(record: UsageRecord) -> Self {
        artemis_domain::Usage {
...
impl From<&artemis_domain::EventValue> for EventValueRecord {
    fn from(event: &artemis_domain::EventValue) -> Self {
        Self(serde_json::to_value(event).unwrap_or_default())
    }
}

impl TryFrom<EventValueRecord> for artemis_domain::EventValue {
...
impl From<&artemis_domain::Role> for RoleRecord {
    fn from(role: &artemis_domain::Role) -> Self {
        match role {
            artemis_domain::Role::System => Self::System,
            artemis_domain::Role::User => Self::User,
            artemis_domain::Role::Assistant => Self::Assistant,
        }
    }
}

impl From<RoleRecord> for artemis_domain::Role {
...
impl From<&artemis_domain::TextMessage> for TextMessageRecord {
    fn from(msg: &artemis_domain::TextMessage) -> Self {
        Self {
...
impl TryFrom<TextMessageRecord> for artemis_domain::TextMessage {
    type Error = anyhow::Error;

    fn try_from(record: TextMessageRecord) -> anyhow::Result<Self> {
        Ok(artemis_domain::TextMessage {
...
impl From<&artemis_domain::ToolValue> for ToolValueRecord {
    fn from(value: &artemis_domain::ToolValue) -> Self {
        match value {
            artemis_domain::ToolValue::Text(text) => Self::Text(text.clone()),
            artemis_domain::ToolValue::AI { value, conversation_id } => Self::AI {
                value: value.clone(),
                conversation_id: conversation_id.into_string(),
            },
            artemis_domain::ToolValue::Image(img) => Self::Image(ImageRecord::from(img)),
            artemis_domain::ToolValue::Empty => Self::Empty,
        }
    }
}

impl TryFrom<ToolValueRecord> for artemis_domain::ToolValue {
    type Error = anyhow::Error;
...
impl From<&artemis_domain::ToolOutput> for ToolOutputRecord {
    fn from(output: &artemis_domain::ToolOutput) -> Self {
        Self {
...
impl TryFrom<ToolOutputRecord> for artemis_domain::ToolOutput {
    type Error = anyhow::Error;
    ...
    fn try_from(record: ToolOutputRecord) -> anyhow::Result<Self> {
        ...
        Ok(artemis_domain::ToolOutput { is_error: record.is_error, values: values? })
    }
}
...
impl From<&artemis_domain::ToolResult> for ToolResultRecord {
    fn from(result: &artemis_domain::ToolResult) -> Self {
        Self {
...
impl TryFrom<ToolResultRecord> for artemis_domain::ToolResult {
    type Error = anyhow::Error;

    fn try_from(record: ToolResultRecord) -> anyhow::Result<Self> {
        Ok(artemis_domain::ToolResult {
            name: record.name.into(),
            call_id: record.call_id.map(Into::into),
...
impl From<&artemis_domain::ContextMessage> for ContextMessageValueRecord {
    fn from(value: &artemis_domain::ContextMessage) -> Self {
        match value {
            artemis_domain::ContextMessage::Text(msg) => Self::Text(TextMessageRecord::from(msg)),
            artemis_domain::ContextMessage::Tool(result) => {
                Self::Tool(ToolResultRecord::from(result))
            }
            artemis_domain::ContextMessage::Image(img) => Self::Image(ImageRecord::from(img)),
        }
    }
}

impl TryFrom<ContextMessageValueRecord> for artemis_domain::ContextMessage {
    type Error = anyhow::Error;
...
impl From<&artemis_domain::MessageEntry> for ContextMessageRecord {
    fn from(msg: &artemis_domain::MessageEntry) -> Self {
        Self {
...
impl TryFrom<ContextMessageRecord> for artemis_domain::MessageEntry {
    type Error = anyhow::Error;

    fn try_from(record: ContextMessageRecord) -> anyhow::Result<Self> {
        Ok(artemis_domain::MessageEntry {
...
impl From<&artemis_domain::ToolDefinition> for ToolDefinitionRecord {
    fn from(def: &artemis_domain::ToolDefinition) -> Self {
        Self {
...
impl TryFrom<ToolDefinitionRecord> for artemis_domain::ToolDefinition {
    type Error = anyhow::Error;

    fn try_from(record: ToolDefinitionRecord) -> anyhow::Result<Self> {
        Ok(artemis_domain::ToolDefinition {
            name: record.name.into(),
...
impl From<&artemis_domain::ToolChoice> for ToolChoiceRecord {
    fn from(choice: &artemis_domain::ToolChoice) -> Self {
        match choice {
            artemis_domain::ToolChoice::None => Self::None,
            artemis_domain::ToolChoice::Auto => Self::Auto,
            artemis_domain::ToolChoice::Required => Self::Required,
            artemis_domain::ToolChoice::Call(name) => Self::Call(ToolNameRecord::from(name)),
        }
    }
}

impl From<ToolChoiceRecord> for artemis_domain::ToolChoice {
...
impl From<&artemis_domain::Effort> for EffortRecord {
    fn from(effort: &artemis_domain::Effort) -> Self {
        match effort {
            artemis_domain::Effort::None => Self::None,
            artemis_domain::Effort::Minimal => Self::Minimal,
            artemis_domain::Effort::Low => Self::Low,
            artemis_domain::Effort::Medium => Self::Medium,
            artemis_domain::Effort::High => Self::High,
            artemis_domain::Effort::XHigh => Self::XHigh,
            artemis_domain::Effort::Max => Self::Max,
        }
    }
}

impl From<EffortRecord> for artemis_domain::Effort {
...
impl From<&artemis_domain::ReasoningConfig> for ReasoningConfigRecord {
    fn from(config: &artemis_domain::ReasoningConfig) -> Self {
        Self {
...
impl From<ReasoningConfigRecord> for artemis_domain::ReasoningConfig {
    fn from(record: ReasoningConfigRecord) -> Self {
        artemis_domain::ReasoningConfig {
...
impl TryFrom<ContextRecord> for Context {
    ...
    fn try_from(record: ContextRecord) -> anyhow::Result<Self> {
        ...
        Ok(Context {
            ...
            temperature: record
                .temperature
                .map(artemis_domain::Temperature::new_unchecked),
            top_p: record.top_p.map(artemis_domain::TopP::new_unchecked),
            top_k: record.top_k.map(artemis_domain::TopK::new_unchecked),
...
pub(super) struct FileChangeMetricsRecord {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    tool: Option<artemis_domain::ToolKind>,
}

impl From<&artemis_domain::FileOperation> for FileChangeMetricsRecord {
    fn from(metrics: &artemis_domain::FileOperation) -> Self {
        Self {
...
impl From<FileChangeMetricsRecord> for artemis_domain::FileOperation {
    fn from(record: FileChangeMetricsRecord) -> Self {
        // Use Write as default tool for old records without tool field
        let tool = record.tool.unwrap_or(artemis_domain::ToolKind::Write);
        Self::new(tool)
...
impl From<&artemis_domain::Metrics> for MetricsRecord {
    fn from(metrics: &artemis_domain::Metrics) -> Self {
        Self {
...
impl From<MetricsRecord> for artemis_domain::Metrics {
    fn from(record: MetricsRecord) -> Self {
        let file_operations: std::collections::HashMap<String, artemis_domain::FileOperation> =
            record
                .files_changed
                .into_iter()
        ...
        let files_accessed = if record.files_accessed.is_empty() {
            ...
            file_operations
                .iter()
                .filter(|(_, op)| op.tool == artemis_domain::ToolKind::Read)
...
impl ConversationRecord {
    ...
    pub fn new(
        conversation: artemis_domain::Conversation,
        workspace_id: artemis_domain::WorkspaceHash,
    ) -> Self {
...
impl TryFrom<ConversationRecord> for artemis_domain::Conversation {
    type Error = anyhow::Error;
    ...
    fn try_from(record: ConversationRecord) -> anyhow::Result<Self> {
        ...
        let metrics = record
            ...
            .map(artemis_domain::Metrics::from)
            .unwrap_or_else(|| {
                artemis_domain::Metrics::default().started_at(record.created_at.and_utc())
            });

        Ok(artemis_domain::Conversation::new(id)
            .context(context)
            .title(record.title)
            .metrics(metrics)
            .metadata(
                artemis_domain::MetaData::new(record.created_at.and_utc())
