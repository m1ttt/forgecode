...
use artemis_domain::{
    AuthMethod, CommandJobSnapshot, CommandKill, CommandOutput, CommandStart,
    FileInfo as FileInfoData, McpServerConfig, ProviderId, URLParamSpec,
};
...
impl ForgeInfra {
    ...
    pub fn new(cwd: PathBuf, config: artemis_config::ForgeConfig) -> Self {
...
impl ForgeInfra {
    ...
    pub fn config(&self) -> anyhow::Result<artemis_config::ForgeConfig> {
...
impl CommandInfra for ForgeInfra {
    ...
    async fn poll_command(&self, job_id: u64) -> anyhow::Result<CommandJobSnapshot> {
        self.command_executor_service.poll_command(job_id).await
    }
    ...
    async fn wait_command(
        ...
        self.command_executor_service
            .wait_command(job_id, timeout_ms)
            .await
    }

    async fn kill_command(&self, job_id: u64) -> anyhow::Result<CommandKill> {
        self.command_executor_service.kill_command(job_id).await
    }
}
...
impl McpServerInfra for ForgeInfra {
    ...
    async fn connect(
        ...
        environment: &artemis_domain::Environment,
...
impl WalkerInfra for ForgeInfra {
    async fn walk(&self, config: artemis_app::Walker) -> anyhow::Result<Vec<artemis_app::WalkedFile>> {
...
impl artemis_domain::ConsoleWriter for ForgeInfra {
