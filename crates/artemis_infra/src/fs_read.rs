...
use anyhow::Result;
use artemis_app::FileReaderInfra;
...
impl FileReaderInfra for ForgeFileReadService {
    async fn read_utf8(&self, path: &Path) -> Result<String> {
        artemis_fs::ForgeFS::read_utf8(path).await
    }
    ...
    async fn read(&self, path: &Path) -> Result<Vec<u8>> {
        artemis_fs::ForgeFS::read(path).await
    }
    ...
    async fn range_read_utf8(
        ...
        start_line: u64,
        end_line: u64,
    ) -> Result<(String, artemis_domain::FileInfo)> {
        artemis_fs::ForgeFS::read_range_utf8(path, start_line, end_line).await
    }
}
