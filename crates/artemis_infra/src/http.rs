...
use artemis_config::{ForgeConfig, TlsBackend, TlsVersion};
...
impl<F: artemis_app::FileWriterInfra + 'static> ForgeHttpInfra<F> {
    /// Creates a new [`ForgeHttpInfra`] from a resolved [`ForgeConfig`].
    pub fn new(config: ForgeConfig, file_writer: Arc<F>) -> Self {
        let http = config.http.unwrap_or(artemis_config::HttpConfig {
    ...
    fn headers(&self, headers: Option<HeaderMap>) -> HeaderMap {
        ...
        headers.insert("X-Title", HeaderValue::from_static("artemis"));
        headers.insert(
            "x-app-version",
...
impl<F: artemis_app::FileWriterInfra + 'static> HttpInfra for ForgeHttpInfra<F> {
...
mod tests {
    ...
    use fake::{Fake, Faker};
    use artemis_app::FileWriterInfra;
    ...
    fn test_sanitize_headers_redacts_sensitive_values() {
        ...
        assert_eq!(
            ...
            Some(&HeaderValue::from_static("artemis"))
        );
        assert_eq!(
