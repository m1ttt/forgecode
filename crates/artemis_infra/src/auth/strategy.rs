...
use artemis_app::{AuthStrategy, OAuthHttpProvider, StrategyFactory};
use artemis_domain::{
...
async fn poll_for_tokens(
    device_code: &artemis_domain::DeviceCode,
    config: &OAuthConfig,
    timeout: Duration,
...
impl ForgeAuthStrategyFactory {
    pub fn new(_environment: artemis_domain::Environment) -> Self {
        Self
    }
}
...
impl StrategyFactory for ForgeAuthStrategyFactory {
    ...
    fn create_auth_strategy(
        ...
        match auth_method {
            artemis_domain::AuthMethod::ApiKey => Ok(AnyAuthStrategy::ApiKey(ApiKeyStrategy::new(
                provider_id,
                required_params,
            ))),
            artemis_domain::AuthMethod::OAuthCode(config) => {
            ...
            artemis_domain::AuthMethod::OAuthDevice(config) => {
            ...
            artemis_domain::AuthMethod::GoogleAdc => Ok(AnyAuthStrategy::GoogleAdc(
                GoogleAdcStrategy::new(provider_id, required_params),
            )),
            artemis_domain::AuthMethod::CodexDevice(config) => Ok(AnyAuthStrategy::CodexDevice(
...
mod tests {
    ...
    fn test_create_auth_strategy_codex_device() {
        ...
        let actual = factory.create_auth_strategy(
            ProviderId::CODEX,
            artemis_domain::AuthMethod::CodexDevice(config),
            vec![],
        );
        assert!(actual.is_ok());
    ...
    fn test_enrich_codex_oauth_credential_uses_id_token_claims() {
        ...
        let expected = Some(&artemis_domain::URLParamValue::from(
