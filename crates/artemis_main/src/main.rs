...
use clap::Parser;
use artemis_api::ForgeAPI;
use artemis_config::ForgeConfig;
use artemis_domain::TitleFormat;
use artemis_main::{Cli, Sandbox, TitleDisplayExt, UI, tracker};
...
async fn run() -> Result<()> {
    ...
    let config =
        ForgeConfig::read().context("Failed to read Artemis configuration from .artemis.toml")?;
...
#[cfg(test)]
mod tests {
    use artemis_main::TopLevelCommand;
    ...
    fn test_stdin_detection_logic() {
        ...
        let cli_with_prompt = Cli::parse_from(["artemis", "--prompt", "existing prompt"]);
        ...
        let cli_no_prompt = Cli::parse_from(["artemis"]);
    ...
    fn test_cli_parsing_with_short_flag() {
        ...
        let cli_with_short_prompt = Cli::parse_from(["artemis", "-p", "short flag prompt"]);
        assert_eq!(
    ...
    fn test_cli_parsing_other_flags_work_with_piping() {
        ...
        let cli_with_flags = Cli::parse_from(["artemis", "--verbose"]);
    ...
    fn test_commit_command_diff_field_initially_none() {
        ...
        let cli = Cli::parse_from(["artemis", "commit", "--preview"]);
