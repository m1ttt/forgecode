...
//! For example: `artemis provider login` instead of `artemis providers login`.
//!
//! NOTE: With every change to this CLI structure, verify that the ZSH plugin
//! remains compatible. The plugin at `shell-plugin/artemis.plugin.zsh` implements
//! shell completion and command shortcuts that depend on the CLI structure.

use std::path::PathBuf;

use clap::{Parser, Subcommand, ValueEnum};
use artemis_domain::{AgentId, ConversationId, Effort, ModelId, ProviderId};

#[derive(Parser)]
#[command(version = env!("CARGO_PKG_VERSION"), name = "artemis")]
pub struct Cli {
    ...
    ///
    /// This field is automatically populated when content is piped to artemis
    ...
    ///
    /// When provided, changes to this directory before starting artemis.
    #[arg(long, short = 'C')]
...
pub enum TopLevelCommand {
    ...
    /// Update artemis to the latest version.
    Update(UpdateArgs),
...
impl From<Scope> for artemis_domain::Scope {
    fn from(value: Scope) -> Self {
        match value {
            Scope::Local => artemis_domain::Scope::Local,
            Scope::User => artemis_domain::Scope::User,
        }
    }
}
...
pub enum ConfigCommand {
    ...
    /// Migrate the legacy ~/artemis directory to ~/.artemis.
    Migrate,
}

/// Arguments for `artemis config set`.
#[derive(Parser, Debug, Clone)]
...
/// Arguments for `artemis config get`.
#[derive(Parser, Debug, Clone)]
...
/// Type-safe subcommands for `artemis config set`.
#[derive(Subcommand, Debug, Clone)]
...
/// Type-safe subcommands for `artemis config get`.
#[derive(Subcommand, Debug, Clone)]
...
pub struct CommitCommandGroup {
    ...
    /// diff content: `git diff | artemis commit --preview`
    #[arg(skip)]
    ...
    /// quotes: `artemis commit fix typo in readme`
    pub text: Vec<String>,
}
...
impl From<DataCommandGroup> for artemis_domain::DataGenerationParameters {
...
mod tests {
    ...
    fn test_data_command_group_conversion() {
        ...
        let actual: artemis_domain::DataGenerationParameters = fixture.into();
        let expected = artemis_domain::DataGenerationParameters {
    ...
    fn test_commit_default_max_diff_size() {
        let fixture = Cli::parse_from(["artemis", "commit", "--preview"]);
    ...
    fn test_commit_custom_max_diff_size() {
        let fixture = Cli::parse_from(["artemis", "commit", "--preview", "--max-diff", "50000"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Commit(commit)) => commit.max_diff_size,
            _ => panic!("Expected Commit command"),
        };
    ...
    #[test]
    fn test_config_list() {
        let fixture = Cli::parse_from(["artemis", "config", "list"]);
        let actual = match fixture.subcommands {
    ...
    fn test_config_get_specific_field() {
        let fixture = Cli::parse_from(["artemis", "config", "get", "model"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Config(config)) => match config.command {
    ...
    fn test_conversation_list() {
        let fixture = Cli::parse_from(["artemis", "conversation", "list"]);
    ...
    fn test_session_alias_list() {
        let fixture = Cli::parse_from(["artemis", "session", "list"]);
        let is_list = match fixture.subcommands {
            Some(TopLevelCommand::Conversation(conversation)) => {
                matches!(conversation.command, ConversationCommand::List { .. })
            }
    ...
    fn test_agent_id_long_flag() {
        let fixture = Cli::parse_from(["artemis", "--agent", "sage"]);
        assert_eq!(fixture.agent, Some(AgentId::new("sage")));
    }

    #[test]
    fn test_agent_id_short_alias() {
        let fixture = Cli::parse_from(["artemis", "--aid", "muse"]);
        assert_eq!(fixture.agent, Some(AgentId::new("muse")));
    }

    #[test]
    fn test_agent_id_with_prompt() {
        let fixture = Cli::parse_from(["artemis", "--agent", "artemis", "-p", "test prompt"]);
        assert_eq!(fixture.agent, Some(AgentId::new("artemis")));
        assert_eq!(fixture.prompt, Some("test prompt".to_string()));
    }

    #[test]
    fn test_agent_id_not_provided() {
        let fixture = Cli::parse_from(["artemis"]);
    ...
    fn test_list_tools_command_with_agent() {
        let fixture = Cli::parse_from(["artemis", "list", "tool", "sage"]);
        let actual = match fixture.subcommands {
    ...
    fn test_list_conversation_command() {
        let fixture = Cli::parse_from(["artemis", "list", "conversation"]);
    ...
    fn test_list_session_alias_command() {
        let fixture = Cli::parse_from(["artemis", "list", "session"]);
        let is_conversation_list = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => matches!(list.command, ListCommand::Conversation),
            _ => false,
        };
    ...
    fn test_list_command_without_custom_flag() {
        let fixture = Cli::parse_from(["artemis", "list", "command"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => match list.command {
    ...
    fn test_list_command_with_custom_flag() {
        let fixture = Cli::parse_from(["artemis", "list", "command", "--custom"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => match list.command {
                ListCommand::Command { custom } => custom,
                _ => false,
    ...
    fn test_cmd_list_with_custom_flag() {
        let fixture = Cli::parse_from(["artemis", "cmd", "list", "--custom"]);
        let actual = match fixture.subcommands {
    ...
    fn test_command_list_with_custom_flag() {
        let fixture = Cli::parse_from(["artemis", "command", "list", "--custom"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Cmd(cmd_group)) => match cmd_group.command {
                CmdCommand::List { custom } => custom,
                _ => false,
    ...
    fn test_info_command_without_porcelain() {
        let fixture = Cli::parse_from(["artemis", "info"]);
        let actual = match fixture.subcommands {
    ...
    fn test_info_command_with_porcelain() {
        let fixture = Cli::parse_from(["artemis", "info", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Info { porcelain, .. }) => porcelain,
            _ => false,
        };
    ...
    fn test_list_agents_without_porcelain() {
        let fixture = Cli::parse_from(["artemis", "list", "agents"]);
        let actual = match fixture.subcommands {
    ...
    fn test_list_agents_with_porcelain() {
        let fixture = Cli::parse_from(["artemis", "list", "agents", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => list.porcelain,
            _ => false,
        };
    ...
    fn test_mcp_list_with_porcelain() {
        let fixture = Cli::parse_from(["artemis", "mcp", "list", "--porcelain"]);
        let actual = match fixture.subcommands {
    ...
    fn test_conversation_list_with_porcelain() {
        let fixture = Cli::parse_from(["artemis", "conversation", "list", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Conversation(conversation)) => match conversation.command {
    ...
    fn test_list_models_with_porcelain() {
        let fixture = Cli::parse_from(["artemis", "list", "models", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => list.porcelain,
            _ => false,
        };
    ...
    fn test_config_list_with_porcelain() {
        let fixture = Cli::parse_from(["artemis", "config", "list", "--porcelain"]);
        let actual = match fixture.subcommands {
    ...
    fn test_prompt_command() {
        let fixture = Cli::parse_from(["artemis", "zsh", "rprompt"]);
    ...
    fn test_cmd_command_with_args() {
        let fixture =
            Cli::parse_from(["artemis", "cmd", "execute", "custom-command", "arg1", "arg2"]);
        let actual = match fixture.subcommands {
    ...
    fn test_commit_with_custom_text() {
        let fixture = Cli::parse_from(["artemis", "commit", "fix", "typo", "in", "readme"]);
        let actual = match fixture.subcommands {
    ...
    fn test_list_skill_command() {
        let fixture = Cli::parse_from(["artemis", "list", "skill"]);
    ...
    fn test_list_skills_alias_command() {
        let fixture = Cli::parse_from(["artemis", "list", "skills"]);
        let is_skill_list = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => matches!(list.command, ListCommand::Skill { .. }),
            _ => false,
        };
    ...
    fn test_list_skill_with_porcelain() {
        let fixture = Cli::parse_from(["artemis", "list", "skill", "--porcelain"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => list.porcelain,
            _ => false,
        };
    ...
    fn test_terminal_plugin_zsh() {
        let fixture = Cli::parse_from(["artemis", "zsh", "plugin"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Zsh(terminal)) => {
    ...
    #[test]
    fn test_zsh_doctor() {
        let fixture = Cli::parse_from(["artemis", "zsh", "doctor"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Zsh(terminal)) => {
    ...
    #[test]
    fn test_zsh_setup() {
        let fixture = Cli::parse_from(["artemis", "zsh", "setup"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Zsh(terminal)) => {
    ...
    fn test_zsh_keyboard() {
        let fixture = Cli::parse_from(["artemis", "zsh", "keyboard"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Zsh(terminal)) => {
    ...
    #[test]
    fn test_zsh_format() {
        let fixture = Cli::parse_from(["artemis", "zsh", "format", "--buffer", "hello world"]);
        let actual = match fixture.subcommands {
    ...
    #[test]
    fn test_setup_alias() {
        let fixture = Cli::parse_from(["artemis", "setup"]);
    ...
    fn test_doctor_alias() {
        let fixture = Cli::parse_from(["artemis", "doctor"]);
    ...
    fn test_install_vscode_extension() {
        let fixture = Cli::parse_from(["artemis", "vscode", "install-extension"]);
        let actual = matches!(
            fixture.subcommands,
    ...
    fn test_list_agent_with_custom_flag() {
        let fixture = Cli::parse_from(["artemis", "list", "agent", "--custom"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => match list.command {
    ...
    fn test_list_agent_without_custom_flag() {
        let fixture = Cli::parse_from(["artemis", "list", "agent"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => match list.command {
                ListCommand::Agent { custom } => custom,
                _ => true,
    ...
    fn test_list_skill_with_custom_flag() {
        let fixture = Cli::parse_from(["artemis", "list", "skill", "--custom"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::List(list)) => match list.command {
    ...
    fn test_update_with_no_confirm() {
        let fixture = Cli::parse_from(["artemis", "update", "--no-confirm"]);
        let actual = match fixture.subcommands {
    ...
    fn test_update_without_no_confirm() {
        let fixture = Cli::parse_from(["artemis", "update"]);
        let actual = match fixture.subcommands {
            Some(TopLevelCommand::Update(args)) => args.no_confirm,
            _ => panic!("Expected Update command"),
        };
