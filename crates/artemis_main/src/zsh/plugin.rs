...
pub fn generate_zsh_plugin() -> Result<String> {
    ...
    for file in artemis_embed::files(&ZSH_PLUGIN_LIB) {
    ...
    generate(Zsh, &mut cmd, "artemis", &mut completions);
...
pub fn generate_zsh_theme() -> Result<String> {
    let mut content =
        super::normalize_script(include_str!("../../../../shell-plugin/artemis.theme.zsh"));

    // Set environment variable to indicate theme is loaded (with timestamp)
    content.push_str("\n_ARTEMIS_THEME_LOADED=$(date +%s)\n");
...
/// * `artemis_editor` - If Some(editor), adds ARTEMIS_EDITOR export to .zshrc
///
/// # Errors
///
/// Returns error if:
...
/// - Invalid artemis markers are found (incomplete or incorrectly ordered)
...
pub fn setup_zsh_integration(
    ...
    const START_MARKER: &str = "# >>> artemis initialize >>>";
    const END_MARKER: &str = "# <<< artemis initialize <<<";
    const FORGE_INIT_CONFIG_RAW: &str = include_str!("../../../../shell-plugin/artemis.setup.zsh");
    ...
    // Build the artemis config block with markers
    ...
    if let Some(editor) = forge_editor {
        ...
        forge_config.push("# To change: update ARTEMIS_EDITOR or remove to use $EDITOR".to_string());
        forge_config.push(format!("export ARTEMIS_EDITOR=\"{}\"", editor));
    }

    forge_config.push(END_MARKER.to_string());

    // Add or update artemis configuration block based on marker state
    let (new_content, config_action) = match marker_state {
        ...
        MarkerState::Invalid { start, end } => {
            ...
            let mut error =
                anyhow::anyhow!("Invalid artemis markers found in {}", zshrc_path.display());
    ...
    Ok(ZshSetupResult {
        message: format!("artemis plugins {}", config_action),
        backup_path,
    })
}
...
mod tests {
    ...
    fn test_run_zsh_doctor_streaming() {
        ...
        unsafe {
            std::env::set_var("ARTEMIS_SKIP_INTERACTIVE", "1");
        }
        ...
        unsafe {
            std::env::remove_var("ARTEMIS_SKIP_INTERACTIVE");
        }
