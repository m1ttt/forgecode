...
impl<S: Services + EnvironmentInfra<Config = artemis_config::ForgeConfig>> AgentExecutor<S> {
    ...
    pub async fn execute(
        ...
        let conversation = if let Some(conversation_id) = conversation_id {
            ...
            let context = artemis_domain::Context::default().initiator("agent".to_string());
