...
impl<S: Services + EnvironmentInfra<Config = artemis_config::ForgeConfig>> GitApp<S> {
    ...
    ///
    /// Sets the user as the author (from git config) and Artemis as the
    /// committer. This properly attributes the commit to both the user and
    /// Artemis using Git's author/committer distinction.
    ///
    /// # Arguments
    ///
    ...
    pub async fn commit(&self, message: String, has_staged_files: bool) -> Result<CommitResult> {
        ...
        // Set Artemis as the committer while keeping the user as the author
        ...
        let commit_command = format!(
            "GIT_COMMITTER_NAME='Artemis' GIT_COMMITTER_EMAIL='noreply@artemis.dev' git commit {flags} -m '{escaped_message}'"
        );
    ...
    async fn generate_message_from_diff(&self, ctx: DiffContext) -> Result<CommitMessageDetails> {
        ...
        let context = artemis_domain::Context::default()
