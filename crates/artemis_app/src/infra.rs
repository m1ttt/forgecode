...
use bytes::Bytes;
use artemis_domain::{
    AuthCodeParams, CommandJobSnapshot, CommandKill, CommandOutput, CommandStart, ConfigOperation,
    Environment, FileInfo, McpServerConfig, OAuthConfig, OAuthTokenResponse, ToolDefinition,
    ToolName, ToolOutput,
};
...
pub trait CommandInfra: Send + Sync {
    ...
    /// Starts a shell command as an asynchronous background job.
    async fn start_command(
        ...
        anyhow::bail!("start_command is not implemented")
    }

    /// Polls the latest buffered output and status for a background job.
    async fn poll_command(&self, _job_id: u64) -> anyhow::Result<CommandJobSnapshot> {
        anyhow::bail!("poll_command is not implemented")
    }
    ...
    async fn wait_command(
        ...
        anyhow::bail!("wait_command is not implemented")
    }

    /// Requests termination of a background job.
    async fn kill_command(&self, _job_id: u64) -> anyhow::Result<CommandKill> {
        anyhow::bail!("kill_command is not implemented")
    }
}
...
pub trait AuthStrategy: Send + Sync {
    ...
    async fn init(&self) -> anyhow::Result<artemis_domain::AuthContextRequest>;

    /// Complete authentication flow
    async fn complete(
        &self,
        context_response: artemis_domain::AuthContextResponse,
...
pub trait StrategyFactory: Send + Sync {
    ...
    fn create_auth_strategy(
        &self,
        provider_id: artemis_domain::ProviderId,
...
///
/// This trait provides access to fully-resolved domain [`artemis_domain::Agent`]
/// values from:
/// 1. Built-in agents (embedded in the application)
/// 2. Global custom agents (from ~/.artemis/agents/ directory)
/// 3. Project-local agents (from .artemis/agents/ directory in current working
///    directory)
/// ## Agent Precedence
