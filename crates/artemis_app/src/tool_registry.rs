...
impl<S: Services + EnvironmentInfra<Config = artemis_config::ForgeConfig>> ToolRegistry<S> {
    ...
    async fn check_tool_permission(
        ...
        if let Some(operation) = operation {
            ...
            if let Some(policy_path) = decision.path {
                use artemis_domain::TitleFormat;
        ...
        if ToolCatalog::contains(&input.name) {
            ...
            if let ToolCatalog::Task(task_input) = tool_input {
                ...
                let conversation_id = session_id
                    .map(|id| artemis_domain::ConversationId::parse(&id))
                    .transpose()
                    .ok()
                    .flatten();
            ...
            if is_restricted && self.check_tool_permission(&tool_input, context).await? {
                ...
                context
                    .send(artemis_domain::TitleFormat::error("Permission Denied"))
                    .await?;
...
impl<S> ToolRegistry<S> {
    fn get_system_tools(
        ...
        env: &Environment,
        model: Option<Model>,
        agents: Vec<artemis_domain::Agent>,
...
#[cfg(test)]
fn create_test_agents() -> Vec<artemis_domain::Agent> {
    use artemis_domain::{Agent, AgentId, ModelId, ProviderId, ToolName};
...
fn create_test_model(
    id: &str,
    modalities: Vec<artemis_domain::InputModality>,
) -> artemis_domain::Model {
    use artemis_domain::{Model, ModelId};
...
fn test_validate_tool_modality_with_image_file_and_vision_model() {
    use artemis_domain::{InputModality, ToolCatalog};

    let vision_model = create_test_model("gpt-4o", vec![InputModality::Text, InputModality::Image]);
    let tool_input = ToolCatalog::Read(artemis_domain::FSRead {
...
fn test_validate_tool_modality_with_no_model() {
    use artemis_domain::ToolCatalog;
...
fn test_validate_tool_modality_with_non_read_tool() {
    ...
    let text_only_model = create_test_model("gpt-3.5-turbo", vec![InputModality::Text]);
    let tool_input = ToolCatalog::Write(artemis_domain::FSWrite {
        file_path: "/home/user/test.png".to_string(),
