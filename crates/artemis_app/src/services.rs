...
use artemis_domain::{
    ...
    ChatCompletionMessage, CommandJobSnapshot, CommandKill, CommandOutput, CommandStart, Context,
    Conversation, ConversationId, File, FileInfo, FileStatus, Image, McpConfig, McpServers, Model,
...
pub struct ShellStartOutput {
    pub output: CommandStart,
    pub shell: String,
    pub description: Option<String>,
}

#[derive(Debug)]
pub struct ShellPollOutput {
    pub output: CommandJobSnapshot,
    pub shell: String,
}

#[derive(Debug)]
pub struct ShellWaitOutput {
    pub output: CommandJobSnapshot,
    pub shell: String,
}

#[derive(Debug)]
pub struct ShellKillOutput {
    pub output: CommandKill,
    pub shell: String,
}
...
pub struct TodoWriteOutput {
    ...
    pub todos: Vec<artemis_domain::Todo>,
}
...
pub trait ProviderService: Send + Sync {
    ...
    async fn get_provider(&self, id: artemis_domain::ProviderId) -> anyhow::Result<Provider<Url>>;
    async fn get_all_providers(&self) -> anyhow::Result<Vec<AnyProvider>>;
    async fn upsert_credential(
        &self,
        credential: artemis_domain::AuthCredential,
    ) -> anyhow::Result<()>;
    async fn remove_credential(&self, id: &artemis_domain::ProviderId) -> anyhow::Result<()>;
...
pub trait AppConfigService: Send + Sync {
    ...
    async fn get_provider_model(
        &self,
        provider_id: Option<&artemis_domain::ProviderId>,
    ) -> anyhow::Result<ModelId>;

    /// Gets the commit configuration (provider and model for commit message
    /// generation).
    async fn get_commit_config(&self) -> anyhow::Result<Option<artemis_domain::ModelConfig>>;

    /// Gets the suggest configuration (provider and model for command
    /// suggestion generation).
    async fn get_suggest_config(&self) -> anyhow::Result<Option<artemis_domain::ModelConfig>>;

    /// Gets the current reasoning effort setting.
    async fn get_reasoning_effort(&self) -> anyhow::Result<Option<artemis_domain::Effort>>;
    ...
    /// all configuration changes; use [`artemis_domain::ConfigOperation`]
    /// variants to describe each mutation.
    async fn update_config(&self, ops: Vec<artemis_domain::ConfigOperation>) -> anyhow::Result<()>;
}
...
pub trait WorkspaceService: Send + Sync {
    ...
    async fn sync_workspace(
        &self,
        path: PathBuf,
    ) -> anyhow::Result<artemis_stream::MpscStream<anyhow::Result<SyncProgress>>>;
...
pub trait FsPatchService: Send + Sync {
    ...
    async fn multi_patch(
        &self,
        path: String,
        edits: Vec<artemis_domain::PatchEdit>,
    ) -> anyhow::Result<PatchOutput>;
}
...
pub trait ImageReadService: Send + Sync {
    ...
    async fn read_image(&self, path: String) -> anyhow::Result<artemis_domain::Image>;
}
...
pub trait FsSearchService: Send + Sync {
    ...
    /// * `Err(_)` - Search error
    async fn search(&self, params: artemis_domain::FSSearch) -> anyhow::Result<Option<SearchResult>>;
}
...
pub trait ShellService: Send + Sync {
    ...
    async fn start(
        ...
        anyhow::bail!(
            "ShellService::start is not implemented for command '{}' in '{}'",
            command,
            cwd.display()
        )
    }

    async fn poll(&self, job_id: u64, _keep_ansi: bool) -> anyhow::Result<ShellPollOutput> {
        anyhow::bail!("ShellService::poll is not implemented for job {}", job_id)
    }
    ...
    async fn wait(
        ...
        anyhow::bail!("ShellService::wait is not implemented for job {}", job_id)
    }

    async fn kill(&self, job_id: u64) -> anyhow::Result<ShellKillOutput> {
        anyhow::bail!("ShellService::kill is not implemented for job {}", job_id)
    }
}
...
pub trait AgentRegistry: Send + Sync {
    ...
    async fn get_agents(&self) -> anyhow::Result<Vec<artemis_domain::Agent>>;

    /// Get lightweight metadata for all agents without requiring a configured
    /// provider or model
    async fn get_agent_infos(&self) -> anyhow::Result<Vec<artemis_domain::AgentInfo>>;

    /// Get agent by ID (from registry store)
    async fn get_agent(&self, agent_id: &AgentId) -> anyhow::Result<Option<artemis_domain::Agent>>;
...
pub trait CommandLoaderService: Send + Sync {
    ...
    async fn get_commands(&self) -> anyhow::Result<Vec<artemis_domain::Command>>;
}
...
pub trait PolicyService: Send + Sync {
    ...
    async fn check_operation_permission(
        &self,
        operation: &artemis_domain::PermissionOperation,
...
pub trait SkillFetchService: Send + Sync {
    ...
    async fn fetch_skill(&self, skill_name: String) -> anyhow::Result<artemis_domain::Skill>;
    ...
    async fn list_skills(&self) -> anyhow::Result<Vec<artemis_domain::Skill>>;
}
...
impl<I: Services> ShellService for I {
    ...
    async fn start(
        ...
        self.shell_service()
            .start(command, cwd, env_vars, description)
            .await
    }

    async fn poll(&self, job_id: u64, keep_ansi: bool) -> anyhow::Result<ShellPollOutput> {
        self.shell_service().poll(job_id, keep_ansi).await
    }
    ...
    async fn wait(
        ...
        self.shell_service()
            .wait(job_id, timeout_ms, keep_ansi)
            .await
    }

    async fn kill(&self, job_id: u64) -> anyhow::Result<ShellKillOutput> {
        self.shell_service().kill(job_id).await
    }
}
