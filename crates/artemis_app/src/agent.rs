...
#[async_trait::async_trait]
impl<T: Services + EnvironmentInfra<Config = artemis_config::ForgeConfig>> AgentService for T {
    async fn chat_agent(
        &self,
        id: &ModelId,
        context: Context,
...
impl AgentExt for Agent {
    fn apply_config(self, config: &ForgeConfig) -> Agent {
        ...
        if let Some(ref workflow_compact) = config.compact {
            // Convert artemis_config::Compact to artemis_domain::Compact, then merge.
        ...
        if let Some(ref config_reasoning) = config.reasoning {
            use artemis_config::Effort as ConfigEffort;
...
#[cfg(test)]
mod tests {
    use artemis_config::{Effort as ConfigEffort, ReasoningConfig as ConfigReasoningConfig};
    use artemis_domain::{AgentId, Effort, ModelId, ProviderId, ReasoningConfig};
