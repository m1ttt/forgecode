...
impl<S: AgentService + EnvironmentInfra<Config = artemis_config::ForgeConfig>> Orchestrator<S> {
    pub fn new(
        services: Arc<S>,
        conversation: Conversation,
        agent: Agent,
    ...
    fn should_yield_after_tool_calls(tool_call_records: &[(ToolCallFull, ToolResult)]) -> bool {
        tool_call_records.iter().any(|(tool_call, result)| {
            tool_call.name == ToolKind::Shell.name() && !result.is_error()
        })
    }
    ...
    pub async fn run(&mut self) -> anyhow::Result<()> {
        ...
        while !should_yield {
            ...
            let should_yield_for_background_shell =
                Self::should_yield_after_tool_calls(&tool_call_records);
            if should_yield_for_background_shell {
                should_yield = true;
            }
            ...
            // it adds messages
            if should_yield && !should_yield_for_background_shell {
