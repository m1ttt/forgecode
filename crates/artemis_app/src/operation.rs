...
use artemis_display::DiffFormat;
use artemis_domain::{
...
use crate::{
    ...
    ReadOutput, ResponseContext, SearchResult, ShellKillOutput, ShellOutput, ShellPollOutput,
...
pub enum ToolOperation {
    ...
    ShellStart {
        output: ShellStartOutput,
    },
    ShellPoll {
        output: ShellPollOutput,
    },
    ShellWait {
        output: ShellWaitOutput,
    },
    ShellKill {
        output: ShellKillOutput,
    },
    FollowUp {
    ...
    Skill {
        output: artemis_domain::Skill,
    },
    TodoWrite {
        before: Vec<artemis_domain::Todo>,
        after: Vec<artemis_domain::Todo>,
    },
    TodoRead {
        output: Vec<artemis_domain::Todo>,
    },
}
...
fn create_validation_warning(path: &str, errors: &[artemis_domain::SyntaxError]) -> Element {
    Element::new("warning")
...
impl ToolOperation {
    /// Converts this tool operation into a [`artemis_domain::ToolOutput`].
    ///
    /// # Arguments
        ...
        match self {
            ToolOperation::FsRead { input, output } => {
                ...
                if let Some(image) = output.content.as_image() {
                    ...
                    *metrics = metrics.clone().insert(
                    ...
                    return artemis_domain::ToolOutput::image(image.clone());
                }
                ...
                *metrics = metrics.clone().insert(
                ...
                artemis_domain::ToolOutput::text(elm)
            }
            ...
            ToolOperation::FsRemove { input, output } => {
                ...
                *metrics = metrics.clone().insert(
                ...
                artemis_domain::ToolOutput::text(elem)
            }
            ...
            ToolOperation::CodebaseSearch { output } => {
                ...
                if output.queries.is_empty() || total_results == 0 {
                    ...
                    for query_result in &output.queries {
                        ...
                        for data in &query_result.results {
                            if let artemis_domain::NodeData::FileChunk(file_chunk) = &data.node {
                ...
                artemis_domain::ToolOutput::text(root)
            }
            ...
            ToolOperation::Shell { output } => {
                ...
                artemis_domain::ToolOutput::text(parent_elem)
            }
            ToolOperation::ShellStart { output } => {
                let job_id = output.output.job_id;
                let mut parent_elem = Element::new("shell_job")
                    .attr("job_id", job_id)
                    .attr("command", &output.output.command)
                    .attr("shell", &output.shell)
                    .attr("status", "started");
                ...
                parent_elem = parent_elem.append(
                    Element::new("next_steps")
                        .text(format!(
                            "Job {job_id} is running in the background. \
                            To check progress use shell_poll with job_id={job_id}. \
                            To wait for completion use shell_wait with job_id={job_id}. \
                            To terminate it use shell_kill with job_id={job_id}. \
                            Do NOT use raw shell commands like `wait`, `jobs`, `ps` or `sleep` to monitor this job."
                        ))
                );
            ...
            ToolOperation::ShellPoll { output } => {
                let mut parent_elem = Element::new("shell_output")
                    .attr("job_id", output.output.job_id)
                    .attr("command", &output.output.output.command)
                    .attr("shell", &output.shell);

                parent_elem = parent_elem.attr("running", output.output.running);

                if let Some(exit_code) = output.output.output.exit_code {
                    parent_elem = parent_elem.attr("exit_code", exit_code);
                }
            ...
            ToolOperation::ShellKill { output } => {
                let parent_elem = Element::new("shell_kill")
                    .attr("job_id", output.output.job_id)
                    .attr("shell", output.shell)
                    .attr("killed", output.output.killed);
                artemis_domain::ToolOutput::text(parent_elem)
            }
            ...
            ToolOperation::TodoWrite { before, after } => {
                ...
                let before_map: std::collections::HashMap<&str, &artemis_domain::Todo> =
...
mod tests {
    ...
    use artemis_domain::{FSRead, FileInfo, ToolValue};
    ...
    fn to_value(output: artemis_domain::ToolOutput) -> String {
    ...
    fn test_syntax_errors(errors: Vec<(u32, u32, &str)>) -> Vec<artemis_domain::SyntaxError> {
        use artemis_domain::SyntaxError;
    ...
    mod sem_search_helpers {
        use fake::{Fake, Faker};
        use artemis_domain::{CodebaseQueryResult, CodebaseSearchResults, FileChunk, Node, NodeData};
    ...
    fn test_shell_output_no_truncation() {
        let fixture = ToolOperation::Shell {
            output: ShellOutput {
                output: artemis_domain::CommandOutput {
    ...
    fn test_fs_remove_success() {
        let fixture = ToolOperation::FsRemove {
            input: artemis_domain::FSRemove { path: "/home/user/file_to_delete.txt".to_string() },
    ...
    fn test_fs_search_files_with_matches_mode() {
        let fixture = ToolOperation::FsSearch {
            input: artemis_domain::FSSearch {
                ...
                output_mode: Some(artemis_domain::OutputMode::FilesWithMatches),
                ..Default::default()
            },
            output: Some(SearchResult {
                matches: vec![
    ...
    fn test_fs_search_count_mode() {
        let fixture = ToolOperation::FsSearch {
            input: artemis_domain::FSSearch {
                path: Some("/home/user/project".to_string()),
                pattern: "test".to_string(),
                output_mode: Some(artemis_domain::OutputMode::Count),
                ..Default::default()
            },
            output: Some(SearchResult {
                matches: vec![
    ...
    fn test_fs_undo_no_changes() {
        let fixture = ToolOperation::FsUndo {
            input: artemis_domain::FSUndo { path: "/home/user/unchanged_file.txt".to_string() },
    ...
    fn test_fs_undo_file_created() {
        let fixture = ToolOperation::FsUndo {
            input: artemis_domain::FSUndo { path: "/home/user/new_file.txt".to_string() },
            output: FsUndoOutput {
                before_undo: None,
    ...
    fn test_fs_undo_file_removed() {
        let fixture = ToolOperation::FsUndo {
            input: artemis_domain::FSUndo { path: "/home/user/deleted_file.txt".to_string() },
            output: FsUndoOutput {
                before_undo: Some(
    ...
    fn test_fs_undo_file_restored() {
        let fixture = ToolOperation::FsUndo {
            input: artemis_domain::FSUndo { path: "/home/user/restored_file.txt".to_string() },
            output: FsUndoOutput {
    ...
    fn test_fs_undo_success() {
        let fixture = ToolOperation::FsUndo {
            input: artemis_domain::FSUndo { path: "/home/user/test.txt".to_string() },
            output: FsUndoOutput {
    ...
    fn test_skill_operation() {
        let fixture = ToolOperation::Skill {
            output: artemis_domain::Skill::new(
                "test-skill",
                "This is a test skill command with instructions",
                "A test skill for demonstration",
            )
            .path("/home/user/.artemis/skills/test-skill")
            .resources(vec![
                PathBuf::from("/home/user/.artemis/skills/test-skill/resource1.txt"),
                PathBuf::from("/home/user/.artemis/skills/test-skill/resource2.md"),
            ]),
        };
    ...
    fn test_fs_read_image_with_vision_model() {
        ...
        match &actual.values[0] {
            artemis_domain::ToolValue::Image(_) => (), // Expected
