...
pub struct Runner {
    ...
    config: artemis_config::ForgeConfig,
    env: Environment,
}
...
impl Runner {
    fn new(setup: &TestContext) -> Self {
        ...
        artemis_embed::register_templates(&mut hb, &TEMPLATE_DIR);
...
impl AgentService for Runner {
    async fn chat_agent(
        &self,
        _id: &artemis_domain::ModelId,
        context: artemis_domain::Context,
        _provider_id: Option<ProviderId>,
    ) -> artemis_domain::ResultStream<ChatCompletionMessage, anyhow::Error> {
    ...
    async fn call(
        &self,
        _: &artemis_domain::Agent,
        _: &artemis_domain::ToolCallContext,
        test_call: artemis_domain::ToolCallFull,
    ) -> artemis_domain::ToolResult {
...
impl TemplateService for Runner {
    ...
    async fn render_template<V: serde::Serialize + Send + Sync>(
        &self,
        template: artemis_domain::Template<V>,
        object: &V,
...
impl AttachmentService for Runner {
    async fn attachments(&self, _url: &str) -> anyhow::Result<Vec<artemis_domain::Attachment>> {
...
impl SkillFetchService for Runner {
    async fn fetch_skill(&self, _skill_name: String) -> anyhow::Result<artemis_domain::Skill> {
