...
async fn test_empty_responses() {
    ...
    ctx.config.retry = Some(artemis_config::RetryConfig {
...
async fn test_no_tool_call_start_end_responses_for_agent_tools() {
    // Call an agent tool (using "artemis" which is configured as an agent in the
    // default workflow)
    let agent_tool_call = ToolCallFull::new("artemis")
...
async fn test_reasoning_should_be_in_context() {
    ...
    ctx.agent = ctx
        .agent
        .reasoning(ReasoningConfig::default().effort(artemis_domain::Effort::High));
...
async fn test_reasoning_not_supported_when_disabled() {
    ...
    ctx.agent = ctx.agent.reasoning(
        ReasoningConfig::default()
            .effort(artemis_domain::Effort::High)
...
#[tokio::test]
async fn test_shell_tool_yields_turn_after_successful_background_start() {
    use artemis_domain::{Metrics, Todo, TodoStatus};

    let fixture = ToolCallFull::new("shell").arguments(ToolCallArguments::from(json!({
        "command": "nmap -sV -sC scanme.nmap.org"
    })));
    let actual = ToolResult::new("shell").output(Ok(ToolOutput::text(
        "<shell_start><job_id>7</job_id><status>running</status></shell_start>",
    )));

    let mut ctx = TestContext::default()
        .mock_tool_call_responses(vec![(fixture.clone(), actual)])
        .mock_assistant_responses(vec![
            ChatCompletionMessage::assistant("Launching scan").tool_calls(vec![fixture.into()]),
            ChatCompletionMessage::assistant("This response should not be consumed")
                .finish_reason(FinishReason::Stop),
        ])
        .initial_metrics(Metrics::default().todos(vec![
            Todo::new("Realizar escaneo de puertos a scanme.nmap.org").status(TodoStatus::Pending),
        ]));

    ctx.run("Haz un scaneo a scanme.nmap.org").await.unwrap();

    let fixture = ctx.output.context_messages();
    let actual = fixture
        .iter()
        .filter(|message| message.has_role(Role::Assistant))
        .count();
