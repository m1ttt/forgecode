...
use artemis_domain::{ChatResponseContent, Environment, TitleFormat, ToolCatalog};
...
impl FormatContent for ToolCatalog {
    fn to_content(&self, env: &Environment) -> Option<ChatResponseContent> {
        ...
        match self {
            ...
            ToolCatalog::ShellExec(input) => Some(
                TitleFormat::debug(format!("Execute [{}]", env.shell))
                    .sub_title(&input.command)
                    .into(),
            ),
            ToolCatalog::ShellPoll(input) => Some(
                TitleFormat::debug("Shell Poll")
                    .sub_title(format!("job {}", input.job_id))
                    .into(),
            ),
            ToolCatalog::ShellWait(input) => Some(
                TitleFormat::debug("Shell Wait")
                    .sub_title(format!("job {}", input.job_id))
                    .into(),
            ),
            ToolCatalog::ShellKill(input) => Some(
                TitleFormat::debug("Shell Kill")
                    .sub_title(format!("job {}", input.job_id))
                    .into(),
            ),
