...
where
    S: EnvironmentInfra<Config = artemis_config::ForgeConfig>
...
mod tests {
    ...
    impl ProviderService for MockServices {
        ...
        async fn models(&self, _provider: Provider<Url>) -> Result<Vec<artemis_domain::Model>> {
            Ok(vec![])
        }
        ...
        async fn get_all_providers(&self) -> Result<Vec<artemis_domain::AnyProvider>> {
            Ok(vec![])
        }
    ...
    impl AppConfigService for MockServices {
        ...
        async fn get_commit_config(&self) -> Result<Option<artemis_domain::ModelConfig>> {
            Ok(None)
        }

        async fn get_suggest_config(&self) -> Result<Option<artemis_domain::ModelConfig>> {
            Ok(None)
        }

        async fn get_reasoning_effort(&self) -> Result<Option<artemis_domain::Effort>> {
            Ok(None)
        }

        async fn update_config(&self, _ops: Vec<artemis_domain::ConfigOperation>) -> Result<()> {
            Ok(())
        }
    }
