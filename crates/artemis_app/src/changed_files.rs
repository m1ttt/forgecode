...
use artemis_domain::{Agent, ContextMessage, Conversation, Role, TextMessage};
...
impl<S: FsReadService + EnvironmentInfra<Config = artemis_config::ForgeConfig>> ChangedFiles<S> {
...
mod tests {
            ...
            self.files
                ...
                .map(|content| {
                    ...
                    ReadOutput {
                        ...
                        info: artemis_domain::FileInfo::new(1, 1, 1, hash),
                    }
                })
    ...
    impl EnvironmentInfra for TestServices {
        ...
        fn get_config(&self) -> anyhow::Result<artemis_config::ForgeConfig> {
            Ok(artemis_config::ForgeConfig { max_parallel_file_reads: 4, ..Default::default() })
        }
