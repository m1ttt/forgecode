...
use artemis_domain::{ContextMessage, Image};
...
impl TryFrom<artemis_domain::Context> for Request {
    type Error = anyhow::Error;
    fn try_from(request: artemis_domain::Context) -> std::result::Result<Self, Self::Error> {
        let system_messages = request
            .messages
            .iter()
            .filter_map(|msg| match &**msg {
                ContextMessage::Text(msg) if msg.has_role(artemis_domain::Role::System) => {
                    Some(SystemMessage {
            ...
            if reasoning.enabled == Some(true) {
                if let Some(budget) = reasoning.max_tokens {
                    ...
                    let output_effort = match effort {
                        artemis_domain::Effort::Low => OutputEffort::Low,
                        artemis_domain::Effort::High => OutputEffort::High,
                        artemis_domain::Effort::Max => OutputEffort::Max,
                        // Map unsupported variants to the nearest Anthropic-valid effort.
                        artemis_domain::Effort::None | artemis_domain::Effort::Minimal => {
                            OutputEffort::Low
                        }
                        artemis_domain::Effort::Medium => OutputEffort::Medium,
                        artemis_domain::Effort::XHigh => OutputEffort::Max,
                    };
        ...
        Ok(Self {
            messages: request
                .messages
                .into_iter()
                .filter(|message| !message.has_role(artemis_domain::Role::System))
            ...
            output_format: request.response_format.and_then(|rf| match rf {
                artemis_domain::ResponseFormat::Text => {
                    // Anthropic doesn't have a "text" output format, so we skip it
                    None
                }
                artemis_domain::ResponseFormat::JsonSchema(schema) => {
        ...
        Ok(match value {
            ContextMessage::Text(chat_message) => {
                ...
                match chat_message.role {
                    artemis_domain::Role::User => Message { role: Role::User, content },
                    artemis_domain::Role::Assistant => Message { role: Role::Assistant, content },
                    artemis_domain::Role::System => {
                        ...
                        return Err(
                            artemis_domain::Error::UnsupportedRole("System".to_string()).into()
                        );
                    }
                }
            }
...
impl TryFrom<artemis_domain::ToolCallFull> for Content {
    type Error = anyhow::Error;
    fn try_from(value: artemis_domain::ToolCallFull) -> std::result::Result<Self, Self::Error> {
        let call_id = value
            .call_id
            .as_ref()
            .ok_or(artemis_domain::Error::ToolCallMissingId)?;
...
impl TryFrom<artemis_domain::ToolResult> for Content {
    type Error = anyhow::Error;
    fn try_from(value: artemis_domain::ToolResult) -> std::result::Result<Self, Self::Error> {
        let call_id = value
            .call_id
            .as_ref()
            .ok_or(artemis_domain::Error::ToolCallMissingId)?;
...
impl From<artemis_domain::ToolChoice> for ToolChoice {
    fn from(value: artemis_domain::ToolChoice) -> Self {
        match value {
            artemis_domain::ToolChoice::Auto => ToolChoice::Auto { disable_parallel_tool_use: None },
            artemis_domain::ToolChoice::Call(tool_name) => {
                ToolChoice::Tool { name: tool_name.to_string(), disable_parallel_tool_use: None }
            }
            artemis_domain::ToolChoice::Required => {
                ToolChoice::Any { disable_parallel_tool_use: None }
            }
            artemis_domain::ToolChoice::None => ToolChoice::Auto { disable_parallel_tool_use: None },
        }
    }
}
...
impl TryFrom<artemis_domain::ToolDefinition> for ToolDefinition {
    type Error = anyhow::Error;
    fn try_from(value: artemis_domain::ToolDefinition) -> std::result::Result<Self, Self::Error> {
        Ok(ToolDefinition {
            name: value.name.to_string(),
...
mod tests {
    ...
    fn test_reasoning_max_tokens_takes_priority_over_effort() {
        ...
        let fixture = Context::default().reasoning(ReasoningConfig {
            effort: Some(artemis_domain::Effort::Low),
            enabled: Some(true),
            max_tokens: Some(8000),
            exclude: None,
        });
