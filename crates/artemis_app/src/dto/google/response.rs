...
impl From<Model> for artemis_domain::Model {
    fn from(value: Model) -> Self {
        ...
        artemis_domain::Model {
            id: artemis_domain::ModelId::new(id),
    ...
    fn try_from(value: EventData) -> Result<Self, Self::Error> {
        match value {
            ...
            EventData::Ping(ping) => {
                ...
                let usage = artemis_domain::Usage { cost: Some(cost), ..Default::default() };
                Ok(ChatCompletionMessage::assistant(artemis_domain::Content::part("")).usage(usage))
            }
            EventData::Unknown(_) => {
                // Silently ignore any other unrecognised events
                Ok(ChatCompletionMessage::assistant(
                    artemis_domain::Content::part(""),
                ))
            }
        }
    }
...
impl From<UsageMetadata> for artemis_domain::Usage {
        ...
        match part {
            Part::Text {
                ...
                if is_thought {
                    ...
                    let mut msg = ChatCompletionMessage::assistant(artemis_domain::Content::part(""))
                        .reasoning(artemis_domain::Content::part(text_content.clone()))
                    ...
                    let mut msg =
                        ChatCompletionMessage::assistant(artemis_domain::Content::part(text_content));
                    if let Some(signature) = thought_signature {
                        msg = msg.thought_signature(signature);
                    }
                    Ok(msg)
            ...
            Part::FunctionCall { function_call, thought_signature } => Ok(
                ChatCompletionMessage::assistant(artemis_domain::Content::part("")).add_tool_call(
                    ToolCallPart {
...
impl TryFrom<Candidate> for ChatCompletionMessage {
    ...
    fn try_from(candidate: Candidate) -> Result<Self, Self::Error> {
        ...
        let mut tool_calls: Vec<artemis_domain::ToolCall> = Vec::new();
        let mut reasoning_details: Option<Vec<artemis_domain::Reasoning>> = None;
        ...
        let mut message = ChatCompletionMessage::assistant(artemis_domain::Content::part(content));
        ...
        if !reasoning_parts.is_empty() {
            message = message.reasoning(artemis_domain::Content::part(reasoning_parts.join("")));
        }
...
impl TryFrom<Response> for ChatCompletionMessage {
    ...
    fn try_from(response: Response) -> Result<Self, Self::Error> {
        ...
        if let Some(candidate) = response.candidates.into_iter().next() {
            ...
            let mut message = ChatCompletionMessage::assistant(artemis_domain::Content::part(""));
...
mod tests {
    ...
    fn test_chat_completion_message_from_part_function_call_generates_id() {
        ...
        match tool_call {
            artemis_domain::ToolCall::Part(part) => {
    ...
    fn test_model_conversion() {
        ...
        let domain_model: artemis_domain::Model = model.into();
        ...
        let domain_model_v2: artemis_domain::Model = model_v2.clone().into();
    ...
    fn test_ping_event_extracts_cost() {
        ...
        let expected = ChatCompletionMessage::assistant(artemis_domain::Content::part(""))
            .usage(artemis_domain::Usage { cost: Some(0.028894), ..Default::default() });
    ...
    fn test_usage_metadata_conversion() {
        ...
        let domain_usage: artemis_domain::Usage = usage.into();
