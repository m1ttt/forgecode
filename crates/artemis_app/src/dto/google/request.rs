...
use artemis_domain::{Context, ContextMessage};
use serde::Serialize;
...
impl From<Context> for Request {
    fn from(context: Context) -> Self {
        ...
        let system_parts: Vec<Part> = context
            .messages
            .iter()
            .filter(|msg| msg.has_role(artemis_domain::Role::System))
            .filter_map(|msg| {
        ...
        let generation_config = Some(GenerationConfig {
            ...
            response_mime_type: context.response_format.as_ref().and_then(|rf| match rf {
                artemis_domain::ResponseFormat::JsonSchema(_) => Some("application/json".to_string()),
                _ => None,
            }),
...
impl From<artemis_domain::ToolChoice> for ToolConfig {
    fn from(choice: artemis_domain::ToolChoice) -> Self {
        let (mode, allowed_function_names) = match choice {
            artemis_domain::ToolChoice::Auto => (FunctionCallingMode::Auto, None),
            artemis_domain::ToolChoice::None => (FunctionCallingMode::None, None),
            artemis_domain::ToolChoice::Required => (FunctionCallingMode::Any, None),
            artemis_domain::ToolChoice::Call(name) => {
...
impl From<artemis_domain::ToolDefinition> for FunctionDeclaration {
    fn from(tool: artemis_domain::ToolDefinition) -> Self {
        let mut parameters =
...
impl From<artemis_domain::TextMessage> for Content {
    fn from(text_message: artemis_domain::TextMessage) -> Self {
        let role = match text_message.role {
            artemis_domain::Role::User => Some(Role::User),
            artemis_domain::Role::Assistant => Some(Role::Model),
            artemis_domain::Role::System => None, // System messages are handled separately
        };
...
impl From<artemis_domain::ToolResult> for Content {
    fn from(tool_result: artemis_domain::ToolResult) -> Self {
        Content {
...
impl From<artemis_domain::Image> for Content {
    fn from(image: artemis_domain::Image) -> Self {
        Content { role: Some(Role::User), parts: vec![Part::from(image)] }
    }
}

impl From<artemis_domain::ToolCallFull> for Part {
    fn from(tool_call: artemis_domain::ToolCallFull) -> Self {
        Part::FunctionCall {
            function_call: FunctionCallData {
...
impl From<artemis_domain::ToolResult> for Part {
    fn from(tool_result: artemis_domain::ToolResult) -> Self {
...
impl From<artemis_domain::Image> for Part {
    fn from(image: artemis_domain::Image) -> Self {
        Part::Image {
            inline_data: ImageSource {
...
#[cfg(test)]
mod tests {
    use artemis_domain::{ToolCallArguments, ToolCallFull, ToolCallId, ToolName, ToolResult};
    ...
    fn test_consecutive_tool_results_grouped() {
        use artemis_domain::{Context, ContextMessage, ModelId};
    ...
    fn test_text_message_conversion() {
        use artemis_domain::{Role, TextMessage};
    ...
    fn test_response_schema_strips_dollar_schema() {
        use artemis_domain::{Context, ResponseFormat};
        use schemars::schema_for;
    ...
    fn test_tool_result_part_conversion() {
        use artemis_domain::{ToolCallId, ToolName, ToolResult};
