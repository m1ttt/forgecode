...
impl From<ReasoningDetail> for artemis_domain::ReasoningDetail {
    fn from(detail: ReasoningDetail) -> Self {
        artemis_domain::ReasoningDetail {
            text: detail.text,
...
impl GitHubCopilotReasoning {
    fn into_reasoning_detail(self) -> artemis_domain::ReasoningDetail {
        artemis_domain::ReasoningDetail {
            text: self.text,
            data: self.data,
            type_of: self.r#type,
                    ...
                    let mut response = match choice {
                        ...
                        Choice::NonStreaming { message, finish_reason, .. } => {
                            ...
                            if let Some(reasoning_details) = &message.reasoning_details {
                                let converted_details: Vec<artemis_domain::ReasoningFull> =
                                    reasoning_details
                                        .clone()
                                        .into_iter()
                                        .map(artemis_domain::ReasoningFull::from)
                                        .collect();

                                resp = resp.add_reasoning_detail(artemis_domain::Reasoning::Full(
                                    converted_details,
                                ));
                                ...
                                for tool_call in tool_calls {
                                    ...
                                    resp = resp.add_tool_call(ToolCallFull {
                                        ...
                                        name: tool_call
                                            ...
                                            .name
                                            .clone()
                                            .ok_or(artemis_domain::Error::ToolCallMissingName)?,
                        ...
                        Choice::Streaming { delta, finish_reason, .. } => {
                            ...
                            if let Some(reasoning_details) = &delta.reasoning_details {
                                let converted_details: Vec<artemis_domain::ReasoningPart> =
                                    reasoning_details
                                        .clone()
                                        .into_iter()
                                        .map(artemis_domain::ReasoningPart::from)
                                        .collect();
                                resp = resp.add_reasoning_detail(artemis_domain::Reasoning::Part(
                                    converted_details,
                                ));
                            } else if let Some(details) = convert_github_copilot_reasoning(
...
mod tests {
    use anyhow::Context;
    use artemis_domain::ChatCompletionMessage;
    ...
    async fn test_responses() -> anyhow::Result<()> {
        let content = artemis_test_kit::fixture!("/src/dto/openai/responses.jsonl").await;
