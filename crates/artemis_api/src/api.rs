...
use artemis_app::{User, UserUsage};
use artemis_domain::{AgentId, Effort, ModelId, ProviderModels};
...
pub trait API: Sync + Send {
    ...
    /// Starts a shell command asynchronously and returns a background job id.
    ...
    /// Polls the current snapshot for a running shell job.
    async fn poll_shell_command(&self, job_id: u64) -> Result<CommandJobSnapshot>;

    /// Waits for a shell job to complete, optionally timing out.
    async fn wait_shell_command(
        &self,
        job_id: u64,
        timeout_ms: Option<u64>,
    ) -> Result<CommandJobSnapshot>;

    /// Attempts to terminate a running shell job.
    async fn kill_shell_command(&self, job_id: u64) -> Result<CommandKill>;
    ...
    /// atomic write. Use [`artemis_domain::ConfigOperation`] variants to describe
