...
use anyhow::Result;
use artemis_app::dto::ToolsOverview;
use artemis_app::{
...
use artemis_domain::{Agent, ConsoleWriter, *};
use artemis_infra::ForgeInfra;
use artemis_repo::ForgeRepo;
use artemis_services::ForgeServices;
use artemis_stream::MpscStream;
...
impl<A, F> ForgeAPI<A, F> {
    ...
    where
        A: Services + EnvironmentInfra<Config = artemis_config::ForgeConfig>,
...
impl ForgeAPI<ForgeServices<ForgeRepo<ForgeInfra>>, ForgeRepo<ForgeInfra>> {
    ...
    pub async fn get_skills_internal(&self) -> Result<Vec<Skill>> {
        use artemis_domain::SkillRepository;
...
> API for ForgeAPI<A, F>
{
    ...
    ) -> Result<artemis_app::CommitResult> {
    ...
    async fn start_shell_command(
        ...
        self.infra
            .start_command(command.to_string(), working_dir, None)
            .await
    }

    async fn poll_shell_command(&self, job_id: u64) -> anyhow::Result<CommandJobSnapshot> {
        self.infra.poll_command(job_id).await
    }

    async fn wait_shell_command(
        &self,
        job_id: u64,
    ...
    async fn kill_shell_command(&self, job_id: u64) -> anyhow::Result<CommandKill> {
    ...
    async fn update_config(&self, ops: Vec<artemis_domain::ConfigOperation>) -> anyhow::Result<()> {
        ...
        let needs_agent_reload = ops
            .iter()
            .any(|op| matches!(op, artemis_domain::ConfigOperation::SetSessionConfig(_)));
    ...
    async fn user_usage(&self) -> Result<Option<UserUsage>> {
        ...
        if let Some(api_key) = provider
            ...
            .and_then(|c| match &c.auth_details {
                artemis_domain::AuthDetails::ApiKey(key) => Some(key.as_str()),
                _ => None,
            })
        {
    ...
    async fn generate_command(&self, prompt: UserPrompt) -> Result<String> {
        use artemis_app::CommandGenerator;
    ...
    async fn sync_workspace(
        &self,
        path: PathBuf,
    ) -> Result<MpscStream<Result<artemis_domain::SyncProgress>>> {
    ...
    ) -> Result<Vec<artemis_domain::Node>> {
        self.services.query_workspace(path, params).await
    }

    async fn list_workspaces(&self) -> Result<Vec<artemis_domain::WorkspaceInfo>> {
    ...
    async fn get_workspace_info(
        &self,
        path: PathBuf,
    ) -> Result<Option<artemis_domain::WorkspaceInfo>> {
        self.services.get_workspace_info(path).await
    }

    async fn delete_workspaces(&self, workspace_ids: Vec<artemis_domain::WorkspaceId>) -> Result<()> {
        self.services.delete_workspaces(&workspace_ids).await
    }

    async fn get_workspace_status(&self, path: PathBuf) -> Result<Vec<artemis_domain::FileStatus>> {
    ...
    async fn create_auth_credentials(&self) -> Result<artemis_domain::WorkspaceAuth> {
        self.services.init_auth_credentials().await
    }

    async fn init_workspace(&self, path: PathBuf) -> Result<artemis_domain::WorkspaceId> {
        self.services.init_workspace(path).await
    }

    async fn migrate_env_credentials(&self) -> Result<Option<artemis_domain::MigrationResult>> {
    ...
    async fn mcp_auth(&self, server_url: &str) -> Result<()> {
        ...
        artemis_infra::mcp_auth(server_url, &env).await
    }

    async fn mcp_logout(&self, server_url: Option<&str>) -> Result<()> {
        let env = self.services.get_environment().clone();
        match server_url {
            Some(url) => artemis_infra::mcp_logout(url, &env).await,
            None => artemis_infra::mcp_logout_all(&env).await,
        }
    }

    async fn mcp_auth_status(&self, server_url: &str) -> Result<String> {
        let env = self.services.get_environment().clone();
        Ok(artemis_infra::mcp_auth_status(server_url, &env).await)
    }
