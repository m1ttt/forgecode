...
use artemis_app::{AuthStrategy, ProviderAuthService, StrategyFactory};
use artemis_domain::{
    ...
    async fn init_provider_auth(
        ...
        let required_params = if matches!(auth_method, AuthMethod::ApiKey | AuthMethod::GoogleAdc) {
            ...
            let provider = providers
                ...
                .ok_or_else(|| artemis_domain::Error::provider_not_available(provider_id.clone()))?;
    ...
    async fn complete_provider_auth(
        ...
        let auth_method = match &auth_context_response {
            AuthContextResponse::ApiKey(response) => {
                ...
                let is_vertex_provider = provider_id == artemis_domain::ProviderId::VERTEX_AI
                    || provider_id == artemis_domain::ProviderId::VERTEX_AI_ANTHROPIC;
                if is_vertex_provider && response.response.api_key.as_ref() == "google_adc_marker" {
                    // Vertex AI uses Google ADC
                    artemis_domain::AuthMethod::google_adc()
                } else {
                    // Regular API key
            ...
            AuthContextResponse::DeviceCode(ctx) => {
                if provider_id == artemis_domain::ProviderId::CODEX {
                        ...
                        | AuthMethod::GoogleAdc => {
                            ...
                            let existing_credential =
                                self.infra.get_credential(&provider.id).await?.ok_or_else(
                                    || artemis_domain::Error::ProviderNotAvailable {
