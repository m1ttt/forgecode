...
use artemis_app::{FileReaderInfra, SyncProgressCounter, WorkspaceStatus, compute_hash};
use artemis_domain::{ApiKey, FileHash, SyncProgress, UserId, WorkspaceId, WorkspaceIndexRepository};
...
/// Extracts [`artemis_domain::FileStatus`] entries with
/// [`artemis_domain::SyncStatus::Failed`] from a slice of file-read results by
/// downcasting errors to [`FileReadError`].
fn extract_failed_statuses<T>(results: &[Result<T>]) -> Vec<artemis_domain::FileStatus> {
    results
        .iter()
        .filter_map(|r| r.as_ref().err())
        .filter_map(|e| e.downcast_ref::<FileReadError>())
        .map(|e| {
            artemis_domain::FileStatus::new(
                e.path.to_string_lossy().into_owned(),
                artemis_domain::SyncStatus::Failed,
            )
        })
        .collect()
}
    ...
    WorkspaceSyncEngine<F, D>
{
        ...
        Fut: std::future::Future<Output = ()> + Send,
    {
        ...
        let added = statuses
            .iter()
            .filter(|s| s.status == artemis_domain::SyncStatus::New)
            .count();
        let deleted = statuses
            .iter()
            .filter(|s| s.status == artemis_domain::SyncStatus::Deleted)
            .count();
        let modified = statuses
            .iter()
            .filter(|s| s.status == artemis_domain::SyncStatus::Modified)
            .count();
        let mut failed_files = statuses
            .iter()
            .filter(|s| s.status == artemis_domain::SyncStatus::Failed)
            .count();
        ...
        if failed_files > 0 {
            Err(artemis_domain::Error::sync_failed(failed_files).into())
        } else {
            Ok(())
        }
    }
    ...
    pub async fn compute_status(&self) -> Result<Vec<artemis_domain::FileStatus>> {
        let results: Vec<Result<FileHash>> = self.read_hashes().collect().await;
    ...
    async fn fetch_remote_hashes(&self) -> anyhow::Result<Vec<FileHash>> {
        ...
        let workspace_files =
            artemis_domain::CodeBase::new(self.user_id.clone(), self.workspace_id.clone(), ());
        self.infra
    ...
    async fn delete_files(&self, files_to_delete: Vec<PathBuf>) -> Result<usize> {
        ...
        let deletion =
            artemis_domain::CodeBase::new(self.user_id.clone(), self.workspace_id.clone(), paths);
        self.infra
    ...
    /// [`artemis_domain::FileUpload`] payload, and uploaded in one request.
        ...
        futures::stream::iter(paths).chunks(batch_size).then(move |batch| {
            ...
            async move {
                ...
                for file_path in &batch {
                    ...
                    files.push(artemis_domain::FileRead::new(
                        file_path.to_string_lossy().into_owned(),
                        content,
                    ));
                }
                let count = files.len();
                let upload = artemis_domain::CodeBase::new(
                    user_id.clone(),
                    workspace_id.clone(),
                    files,
                );
