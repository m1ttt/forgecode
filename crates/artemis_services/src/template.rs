...
use anyhow::Context;
use artemis_app::{EnvironmentInfra, FileReaderInfra, TemplateService};
use artemis_domain::Template;
use futures::future;
...
impl<F: EnvironmentInfra + FileReaderInfra> ForgeTemplateService<F> {
    ...
    async fn get_hb(&self) -> &RwLock<Handlebars<'static>> {
        self.hb
            .get_or_init(|| async { RwLock::new(artemis_app::TemplateEngine::handlebar_instance()) })
            .await
    }
