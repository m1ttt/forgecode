...
use anyhow::Result;
use artemis_app::ProviderService;
...
impl<R> ForgeProviderService<R> {
    ...
    fn render_url_template(
        &self,
        template: &str,
        params: &HashMap<artemis_domain::URLParam, artemis_domain::URLParamValue>,
    ) -> Result<Url> {
        ...
        let handlebars = artemis_app::TemplateEngine::handlebar_instance();
...
#[cfg(test)]
mod tests {
    use artemis_app::domain::ProviderId;
    use artemis_domain::{
    ...
    fn test_provider() -> Provider<Url> {
        Provider {
            ...
            response: Some(artemis_app::domain::ProviderResponse::OpenAI),
            url: Url::parse("https://api.openai.com/v1/chat/completions").unwrap(),
            auth_methods: vec![AuthMethod::ApiKey],
            url_params: vec![],
            credential: Some(AuthCredential {
                id: ProviderId::OPENAI,
                auth_details: AuthDetails::ApiKey(artemis_domain::ApiKey::from(
                    "test-key".to_string(),
                )),
    ...
    fn test_template_provider() -> ProviderTemplate {
        Provider {
            ...
            provider_type: ProviderType::Llm,
            response: Some(artemis_app::domain::ProviderResponse::OpenAI),
            url: Template::<artemis_domain::URLParameters>::new(
            ...
            models: Some(ModelSource::Url(
                Template::<artemis_domain::URLParameters>::new("https://api.openai.com/v1/models"),
            )),
            custom_headers: None,
        }
    }
