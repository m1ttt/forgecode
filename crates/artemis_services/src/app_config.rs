...
use artemis_app::{AppConfigService, EnvironmentInfra};
use artemis_domain::{ConfigOperation, Effort, ModelConfig, ModelId, ProviderId, ProviderRepository};
use tracing::debug;
...
impl<F: ProviderRepository + EnvironmentInfra<Config = artemis_config::ForgeConfig> + Send + Sync>
    AppConfigService for ForgeAppConfigService<F>
{
    async fn get_default_provider(&self) -> anyhow::Result<ProviderId> {
        ...
        config
            ...
            .ok_or_else(|| artemis_domain::Error::NoDefaultSession.into())
    }
    ...
    async fn get_provider_model(
        ...
        let session = config
            .session
            .as_ref()
            .ok_or(artemis_domain::Error::NoDefaultSession)?;
        ...
        if session.provider_id == requested_provider {
            ...
            Err(artemis_domain::Error::NoDefaultSession.into())
        }
    }
    ...
    async fn get_reasoning_effort(&self) -> anyhow::Result<Option<Effort>> {
        ...
        Ok(config
            ...
            .and_then(|r| r.effort)
            .map(|e| match e {
                artemis_config::Effort::None => Effort::None,
                artemis_config::Effort::Minimal => Effort::Minimal,
                artemis_config::Effort::Low => Effort::Low,
                artemis_config::Effort::Medium => Effort::Medium,
                artemis_config::Effort::High => Effort::High,
                artemis_config::Effort::XHigh => Effort::XHigh,
                artemis_config::Effort::Max => Effort::Max,
            }))
    }
...
mod tests {
    ...
    use artemis_config::{ForgeConfig, ModelConfig};
    // Alias to avoid collision with artemis_config::ModelConfig used in test fixtures
    use artemis_domain::ModelConfig as DomainModelConfig;
    use artemis_domain::{
                ...
                providers: vec![
                    Provider {
                        ...
                        credential: Some(artemis_domain::AuthCredential {
                            id: ProviderId::OPENAI,
                            auth_details: artemis_domain::AuthDetails::ApiKey(
                                artemis_domain::ApiKey::from("test-key".to_string()),
                            ),
    ...
    impl ChatRepository for MockInfra {
        async fn chat(
            &self,
            _model_id: &artemis_app::domain::ModelId,
            _context: artemis_app::domain::Context,
            _provider: Provider<Url>,
        ) -> artemis_app::domain::ResultStream<artemis_app::domain::ChatCompletionMessage, anyhow::Error>
        {
        ...
        async fn models(
            &self,
            _provider: Provider<Url>,
        ) -> anyhow::Result<Vec<artemis_app::domain::Model>> {
            Ok(vec![])
        }
    }
        ...
        async fn get_provider(&self, id: ProviderId) -> anyhow::Result<ProviderTemplate> {
            ...
            self.providers
                ...
                .map(|p| Provider {
                    ...
                    url: artemis_domain::Template::<artemis_domain::URLParameters>::new(p.url.as_str()),
                    models: p.models.as_ref().map(|m| match m {
                        ModelSource::Url(url) => ModelSource::Url(artemis_domain::Template::<
                            artemis_domain::URLParameters,
                        >::new(
                            url.as_str()
                        )),
