...
use artemis_app::{CommandInfra, EnvironmentInfra, FileReaderInfra, WalkerInfra, WorkspaceService};
use artemis_domain::{
...
> ForgeWorkspaceService<F, D>
{
    ...
    async fn get_workspace_credentials(&self) -> Result<(artemis_domain::ApiKey, UserId)> {
        let credential = self
            .infra
    ...
    async fn find_workspace_by_path(
        ...
        let mut best_match: Option<(&artemis_domain::WorkspaceInfo, usize)> = None;
    ...
    async fn get_workspace_by_path(
        ...
        path: PathBuf,
        token: &artemis_domain::ApiKey,
    ) -> Result<artemis_domain::WorkspaceInfo> {
...
> WorkspaceService for ForgeWorkspaceService<F, D>
{
    ...
    async fn query_workspace(
        ...
        let workspace = self
            ...
            .await?
            .ok_or(artemis_domain::Error::WorkspaceNotFound)?;

        let search_query =
            artemis_domain::CodeBase::new(user_id, workspace.workspace_id.clone(), params);
    ...
    async fn delete_workspace(&self, workspace_id: &artemis_domain::WorkspaceId) -> Result<()> {
        let (token, _) = self.get_workspace_credentials().await?;
    ...
    /// database.
    async fn delete_workspaces(&self, workspace_ids: &[artemis_domain::WorkspaceId]) -> Result<()> {
    ...
    async fn init_auth_credentials(&self) -> Result<artemis_domain::WorkspaceAuth> {
    ...
    async fn init_workspace(&self, path: PathBuf) -> Result<WorkspaceId> {
        ...
        if is_new {
            ...
            Err(artemis_domain::Error::WorkspaceAlreadyInitialized(workspace_id).into())
        }
    }
}
