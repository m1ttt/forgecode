...
use artemis_domain::Conversation;
use sysinfo::System;
...
/// Returns the value of the tracking environment variable, checking
/// `ARTEMIS_TRACKER` first and falling back to `FORGE_TRACKER` for backward
/// compatibility.
fn tracking_env_var() -> Option<String> {
    std::env::var("ARTEMIS_TRACKER")
        .ok()
        .or_else(|| std::env::var("FORGE_TRACKER").ok())
}
...
mod tests {
    ...
    fn test_tracking_fixture() {
        unsafe {
            std::env::remove_var("ARTEMIS_TRACKER");
            std::env::remove_var("FORGE_TRACKER");
        }
        ...
        unsafe {
            std::env::set_var("ARTEMIS_TRACKER", "false");
        }
        let actual = tracking_enabled();
        let expected = false;
        assert_eq!(actual, expected);
        ...
        unsafe {
            std::env::set_var("ARTEMIS_TRACKER", "true");
        }
        let actual = tracking_enabled();
        let expected = true;
        assert_eq!(actual, expected);
