...
fn json_repair_parse<T>(
    json_str: &str,
) -> std::result::Result<T, artemis_json_repair::JsonRepairError>
where
    T: serde::de::DeserializeOwned,
{
    serde_json::from_str(json_str)
        .map_err(artemis_json_repair::JsonRepairError::JsonError)
        .or_else(|_| {
            let repaired = artemis_json_repair::json_repair(json_str);
            if repaired.is_ok() {
...
mod tests {
    ...
    async fn test_real_example() {
        let message = artemis_test_kit::fixture!("/src/fixtures/tool_call_01.md").await;
