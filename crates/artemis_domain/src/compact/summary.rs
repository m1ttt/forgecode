...
fn extract_tool_info(call: &ToolCallFull, current_todos: &[Todo]) -> Option<SummaryTool> {
    ...
    if let Ok(tool) = ToolCatalog::try_from(call.clone()) {
        return match tool {
            ...
            ToolCatalog::ShellExec(input) => Some(SummaryTool::Shell { command: input.command }),
            ToolCatalog::ShellPoll(input) => Some(SummaryTool::Shell {
                command: format!("poll job {}", input.job_id),
            }),
            ToolCatalog::ShellWait(input) => Some(SummaryTool::Shell {
                command: format!("wait job {}", input.job_id),
            }),
            ToolCatalog::ShellKill(input) => Some(SummaryTool::Shell {
                command: format!("kill job {}", input.job_id),
            }),
